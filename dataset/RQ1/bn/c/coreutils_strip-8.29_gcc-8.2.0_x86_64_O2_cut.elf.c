int64_t sub_401630()
{
    int64_t var_8 = data_60b008;
    /* jump -> data_60b010 */
}

int64_t sub_401646()
{
    int64_t var_8 = 0;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401656()
{
    int64_t var_8 = 1;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401666()
{
    int64_t var_8 = 2;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401676()
{
    int64_t var_8 = 3;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401686()
{
    int64_t var_8 = 4;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401696()
{
    int64_t var_8 = 5;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4016a6()
{
    int64_t var_8 = 6;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4016b6()
{
    int64_t var_8 = 7;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4016c6()
{
    int64_t var_8 = 8;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4016d6()
{
    int64_t var_8 = 9;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4016e6()
{
    int64_t var_8 = 0xa;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4016f6()
{
    int64_t var_8 = 0xb;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401706()
{
    int64_t var_8 = 0xc;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401716()
{
    int64_t var_8 = 0xd;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401726()
{
    int64_t var_8 = 0xe;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401736()
{
    int64_t var_8 = 0xf;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401746()
{
    int64_t var_8 = 0x10;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401756()
{
    int64_t var_8 = 0x11;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401766()
{
    int64_t var_8 = 0x12;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401776()
{
    int64_t var_8 = 0x13;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401786()
{
    int64_t var_8 = 0x14;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401796()
{
    int64_t var_8 = 0x15;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4017a6()
{
    int64_t var_8 = 0x16;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4017b6()
{
    int64_t var_8 = 0x17;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4017c6()
{
    int64_t var_8 = 0x18;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4017d6()
{
    int64_t var_8 = 0x19;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4017e6()
{
    int64_t var_8 = 0x1a;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401806()
{
    int64_t var_8 = 0x1c;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401826()
{
    int64_t var_8 = 0x1e;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401836()
{
    int64_t var_8 = 0x1f;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401846()
{
    int64_t var_8 = 0x20;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401856()
{
    int64_t var_8 = 0x21;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401866()
{
    int64_t var_8 = 0x22;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401876()
{
    int64_t var_8 = 0x23;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401886()
{
    int64_t var_8 = 0x24;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401896()
{
    int64_t var_8 = 0x25;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4018a6()
{
    int64_t var_8 = 0x26;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4018b6()
{
    int64_t var_8 = 0x27;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4018c6()
{
    int64_t var_8 = 0x28;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4018d6()
{
    int64_t var_8 = 0x29;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4018e6()
{
    int64_t var_8 = 0x2a;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4018f6()
{
    int64_t var_8 = 0x2b;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401906()
{
    int64_t var_8 = 0x2c;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401916()
{
    int64_t var_8 = 0x2d;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401926()
{
    int64_t var_8 = 0x2e;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401936()
{
    int64_t var_8 = 0x2f;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401946()
{
    int64_t var_8 = 0x30;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401956()
{
    int64_t var_8 = 0x31;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401966()
{
    int64_t var_8 = 0x32;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401976()
{
    int64_t var_8 = 0x33;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401986()
{
    int64_t var_8 = 0x34;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401996()
{
    int64_t var_8 = 0x35;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4019a6()
{
    int64_t var_8 = 0x36;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4019b6()
{
    int64_t var_8 = 0x37;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4019c6()
{
    int64_t var_8 = 0x38;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4019d6()
{
    int64_t var_8 = 0x39;
    /* tailcall */
    return sub_401630();
}

int64_t sub_4019e6()
{
    int64_t var_8 = 0x3a;
    /* tailcall */
    return sub_401630();
}

int64_t sub_401a06()
{
    int64_t var_8 = 0x3c;
    /* tailcall */
    return sub_401630();
}

int32_t main(int32_t argc, char** argv, char** envp)
{
    int64_t r13;
    int64_t var_10 = r13;
    char r12 = 0;
    char* rdi = *argv;
    int64_t rcx;
    int64_t rdx;
    uint64_t r8;
    ssize_t r9;
    rcx = sub_403790(rdi);
    int32_t rbx;
    int32_t rbp;
    setlocale(6, &data_4094ec[0x1b], rdx, rcx, r8, r9, rbx, rbp);
    bindtextdomain("coreutils", "/home/dongkwan/data/scripts/gnu_…");
    textdomain("coreutils");
    sub_4074f0(sub_4031d0);
    data_60b313 = 0;
    data_60b314 = 0;
    data_60b311 = 0;
    data_60b2fa = 0;
    while (true)
    {
        int32_t rax_1 = getopt_long(argc, argv, "b:c:d:f:nsz", &data_407fa0, nullptr);
        char const* const rsi_1;
        if (rax_1 == 0xffffffff)
        {
            int32_t rax_9 = data_60b314;
            rsi_1 = "you must specify a list of bytes…";
            if (rax_9 != 0)
            {
                bool rdx_4 = rax_9 != 2;
                if ((rax_9 != 2 && r12 != 0))
                {
                    rsi_1 = "an input delimiter may be specif…";
                }
                if ((rax_9 == 2 || (rax_9 != 2 && r12 == 0)))
                {
                    if ((data_60b313 & rdx_4) == 0)
                    {
                        int32_t rsi_2;
                        rsi_2 = rax_9 != 2;
                        sub_402bd0(r13, ((rsi_2 << 2) | ((!(rax_9 - rax_9)) & 2)));
                        if (r12 == 0)
                        {
                            data_60b311 = 9;
                        }
                        if (data_60b300 == 0)
                        {
                            char rax_22 = data_60b311;
                            data_60b2f9 = 0;
                            data_60b300 = 0x60b2f8;
                            data_60b2f8 = rax_22;
                            data_60b308 = 1;
                        }
                        int32_t rax_16 = data_60b2bc;
                        bool cond:4 = rax_16 >= argc;
                        int32_t r12_1;
                        if (rax_16 == argc)
                        {
                            r12_1 = sub_401ee0(&data_4094ea);
                        }
                        else
                        {
                            r12_1 = 1;
                            if ((!cond:4))
                            {
                                do
                                {
                                    r12_1 = (r12_1 & sub_401ee0(argv[rax_16]));
                                    rax_16 = (data_60b2bc + 1);
                                    data_60b2bc = rax_16;
                                } while (rax_16 < argc);
                            }
                        }
                        if ((data_60b2fa != 0 && sub_405c20(stdin) == 0xffffffff))
                        {
                            r12_1 = 0;
                            error(0, *__errno_location(), &data_4094ea);
                        }
                        return (r12_1 ^ 1);
                    }
                    rsi_1 = "suppressing non-delimited lines …";
                }
            }
        }
        else
        {
            bool cond:1_1 = rax_1 <= 0x66;
            if (rax_1 != 0x66)
            {
                if (cond:1_1)
                {
                    if (rax_1 > 0x63)
                    {
                        if (rax_1 != 0x64)
                        {
                            break;
                        }
                        int64_t rdx_2 = data_60b4f0;
                        char rax_7 = *rdx_2;
                        if ((rax_7 == 0 || (rax_7 != 0 && *(rdx_2 + 1) == 0)))
                        {
                            data_60b311 = rax_7;
                            r12 = 1;
                            continue;
                        }
                        if ((rax_7 != 0 && *(rdx_2 + 1) != 0))
                        {
                            rsi_1 = "the delimiter must be a single c…";
                        }
                    }
                    else
                    {
                        if (rax_1 < 0x62)
                        {
                            if (rax_1 == 0xffffff7d)
                            {
                                int64_t var_30 = 0;
                                char const* const var_38 = "Jim Meyering";
                                sub_405830(stdout, &data_40755b, "GNU coreutils", data_60b238);
                                exit(0);
                                /* no return */
                            }
                            if (rax_1 == 0xffffff7e)
                            {
                                sub_4026b0(0);
                                /* no return */
                            }
                            break;
                        }
                        if (data_60b314 == 0)
                        {
                            data_60b314 = 1;
                            r13 = data_60b4f0;
                            continue;
                        }
                    }
                }
                else
                {
                    bool cond:3_1 = rax_1 > 0x7a;
                    if (rax_1 == 0x7a)
                    {
                        data_60b230 = 0;
                        continue;
                    }
                    else if (cond:3_1)
                    {
                        if (rax_1 == 0x80)
                        {
                            int64_t rdx_1 = data_60b4f0;
                            data_60b310 = 1;
                            int64_t rax_3 = 1;
                            if (*rdx_1 != 0)
                            {
                                int64_t rcx_2 = -1;
                                int64_t rdi_3 = rdx_1;
                                while (rcx_2 != 0)
                                {
                                    bool cond:5_1 = 0 != *rdi_3;
                                    rdi_3 = (rdi_3 + 1);
                                    rcx_2 = (rcx_2 - 1);
                                    if ((!cond:5_1))
                                    {
                                        break;
                                    }
                                }
                                rax_3 = ((!rcx_2) - 1);
                            }
                            data_60b308 = rax_3;
                            data_60b300 = sub_405ba0(rdx_1);
                            continue;
                        }
                        else
                        {
                            if (rax_1 != 0x81)
                            {
                                break;
                            }
                            data_60b312 = 1;
                            continue;
                        }
                    }
                    else if (rax_1 == 0x6e)
                    {
                        continue;
                    }
                    else
                    {
                        if (rax_1 != 0x73)
                        {
                            break;
                        }
                        data_60b313 = 1;
                        continue;
                    }
                }
            }
            else if (data_60b314 == 0)
            {
                data_60b314 = 2;
                r13 = data_60b4f0;
                continue;
            }
            if ((rax_1 == 0x66 || ((rax_1 != 0x66 && cond:1_1) && rax_1 <= 0x63)))
            {
                rsi_1 = "only one type of list may be spe…";
            }
        }
        error(0, 0, dcgettext(nullptr, rsi_1, 5));
        break;
    }
    sub_4026b0(1);
    /* no return */
}

int64_t _start(int64_t arg1, int64_t arg2, void (* arg3)()) __noreturn
{
    int64_t rax;
    int64_t var_8 = rax;
    __libc_start_main(main, __return_addr, &arg_8, init, fini, arg3, &var_8);
    /* no return */
}

void sub_401e2b()
{
    return;
}

int64_t sub_401e4a()
{
    int64_t rax;
    int64_t rdx;
    rdx = HIGHD(0);
    rax = LOWD(0);
    return (COMBINE(rdx, rax) / 2);
}

void sub_401e81()
{
    if (data_60b2e8 != 0)
    {
        return;
    }
    while (true)
    {
        int64_t rax_1 = data_60b2f0;
        if (rax_1 >= 0)
        {
            break;
        }
        data_60b2f0 = (rax_1 + 1);
        *(0x60ae40 + ((rax_1 + 1) << 3))();
    }
    sub_401e2b();
    data_60b2e8 = 1;
}

int64_t j_sub_401e4a()
{
    /* tailcall */
    return sub_401e4a();
}

int64_t sub_401ee0(uint64_t arg1)
{
    uint64_t rsi = arg1;
    int64_t rcx = 2;
    void* const rdi = &data_4094ea;
    uint64_t r12;
    uint64_t var_20 = r12;
    int64_t __saved_rbx;
    bool c = &__saved_rbx < 0x18;
    bool z = &__saved_rbx == 0x18;
    while (rcx != 0)
    {
        char temp1_1 = *rsi;
        char temp2_1 = *rdi;
        c = temp1_1 < temp2_1;
        z = temp1_1 == temp2_1;
        rsi = (rsi + 1);
        rdi = (rdi + 1);
        rcx = (rcx - 1);
        if ((!z))
        {
            break;
        }
    }
    FILE* rax_30;
    uint64_t* const r13;
    if ((((!z) && (!c)) - 0) != 0)
    {
        rax_30 = fopen(arg1, &data_40764e);
        r13 = rax_30;
    }
    else
    {
        data_60b2fa = 1;
        r13 = stdin;
    }
    if (((((!z) && (!c)) - 0) == 0 || ((((!z) && (!c)) - 0) != 0 && rax_30 != 0)))
    {
        sub_403280(r13, 2);
        bool cond:2_1 = data_60b314 == 1;
        data_60b328 = data_60b4e0;
        char rax_19;
        if (cond:2_1)
        {
            int64_t rbx_4 = 0;
            char r12_4 = 0;
            int32_t rbp_3;
            uint32_t rsi_10;
            while (true)
            {
            label_4024b0:
                char* rax_35 = r13[1];
                int64_t rax_37;
                if (rax_35 < r13[2])
                {
                    do
                    {
                        rsi_10 = data_60b230;
                        r13[1] = &rax_35[1];
                        rbp_3 = *rax_35;
                        if (rbp_3 == rsi_10)
                        {
                            break;
                        }
                    label_4024d8:
                        int64_t rax_36 = data_60b328;
                        rbx_4 = (rbx_4 + 1);
                        if (rbx_4 > *(rax_36 + 8))
                        {
                            rax_36 = (rax_36 + 0x10);
                            data_60b328 = rax_36;
                        }
                        rax_37 = *rax_36;
                        if (rbx_4 < rax_37)
                        {
                            break;
                        }
                        char rdx_16 = data_60b310;
                        FILE* rdi_18 = stdout;
                        if (rdx_16 != 0)
                        {
                            rax_37 = rbx_4 == rax_37;
                            r12_4 = (r12_4 & rax_37);
                            if (r12_4 != 0)
                            {
                                fwrite_unlocked(data_60b300, 1, data_60b308, stdout);
                                rdi_18 = stdout;
                            }
                            else
                            {
                                r12_4 = rdx_16;
                            }
                        }
                        char* rax_38 = stdout[5];
                        if (rax_38 >= stdout[6])
                        {
                            __overflow(rdi_18, rbp_3);
                            goto label_4024b0;
                        }
                        stdout[5] = &rax_38[1];
                        *rax_38 = rbp_3;
                        rax_35 = r13[1];
                    } while (rax_35 < r13[2]);
                    if (((rbp_3 != rsi_10 && rbp_3 != rsi_10) && rbx_4 < rax_37))
                    {
                        continue;
                    }
                }
                if ((rax_35 >= r13[2] || ((rax_35 < r13[2] && rbp_3 != rsi_10) && (!(rbp_3 != rsi_10 && rbx_4 < rax_37)))))
                {
                    rsi_10 = data_60b230;
                    rbp_3 = __uflow(r13);
                    if (rbp_3 != rsi_10)
                    {
                        break;
                    }
                }
                FILE* rdi_21 = stdout;
                char* rax_40 = stdout[5];
                if (rax_40 >= stdout[6])
                {
                    __overflow(rdi_21, rsi_10);
                }
                else
                {
                    stdout[5] = &rax_40[1];
                    *rax_40 = rsi_10;
                }
                rbx_4 = 0;
                r12_4 = 0;
                data_60b328 = data_60b4e0;
            }
            if (rbp_3 != 0xffffffff)
            {
                goto label_4024d8;
            }
            if (rbx_4 != 0)
            {
                FILE* rdi_20 = stdout;
                char* rdx_18 = stdout[5];
                if (rdx_18 >= stdout[6])
                {
                    __overflow(rdi_20, rsi_10);
                    rax_19 = *r13;
                }
                else
                {
                    stdout[5] = &rdx_18[1];
                    *rdx_18 = rsi_10;
                    rax_19 = *r13;
                }
                goto label_402234;
            }
        }
        else
        {
            char* rax_2 = r13[1];
            uint32_t rax_18;
            uint32_t rdi_2;
            if (rax_2 >= r13[2])
            {
                rax_18 = __uflow(r13);
                rdi_2 = rax_18;
            }
            else
            {
                r13[1] = &rax_2[1];
                rdi_2 = *rax_2;
            }
            if ((rax_2 < r13[2] || (rax_2 >= r13[2] && rax_18 != 0xffffffff)))
            {
                int64_t rbx_1 = 1;
                ungetc(rdi_2, r13);
                int64_t r15;
                r15 = **&data_60b328 > 1;
                r15 = (r15 ^ data_60b313);
                bool rbp_1 = false;
                int32_t rdx_2 = 0;
                char var_3d_1 = r15;
                while (true)
                {
                label_401f94:
                    r12 = rbx_1 == 1;
                    r12 = (r12 & var_3d_1);
                    int64_t rax_4;
                    if (r12 == 0)
                    {
                        while (true)
                        {
                            rax_4 = data_60b328;
                            r12 = rdx_2;
                        label_401fad:
                            char rax_5;
                            bool rcx_2;
                            uint32_t rdx_11;
                            int32_t rbp_2;
                            bool rdi_4;
                            uint32_t r15_1;
                            if (*rax_4 > rbx_1)
                            {
                            label_402094:
                                char* rax_9 = r13[1];
                                if (rax_9 < r13[2])
                                {
                                    r13[1] = &rax_9[1];
                                    r15_1 = *rax_9;
                                }
                                else
                                {
                                    r15_1 = __uflow(r13);
                                }
                                rdx_11 = data_60b311;
                                rax_5 = data_60b230;
                                rcx_2 = rdx_11;
                                rdi_4 = rdx_11;
                                if (rdx_11 != r15_1)
                                {
                                    rdx_2 = rax_5;
                                    if ((r15_1 != rdx_2 && r15_1 != 0xffffffff))
                                    {
                                        r12 = r15_1;
                                        goto label_402094;
                                    }
                                }
                            }
                            else
                            {
                                if (rbp_1 != 0)
                                {
                                    fwrite_unlocked(data_60b300, 1, data_60b308, stdout);
                                }
                            label_40203f:
                                char* rax_7 = r13[1];
                                if (rax_7 < r13[2])
                                {
                                    r13[1] = &rax_7[1];
                                    rbp_2 = *rax_7;
                                }
                                else
                                {
                                    rbp_2 = __uflow(r13);
                                }
                                rdx_11 = data_60b311;
                                rax_5 = data_60b230;
                                rcx_2 = rdx_11;
                                rdi_4 = rdx_11;
                                r15_1 = rdx_11;
                                if (rbp_2 == rdx_11)
                                {
                                    rbp_1 = true;
                                }
                            }
                            char* rax_24;
                            int32_t rax_26;
                            uint32_t rcx_3;
                            int32_t rsi_3;
                            bool r8_1;
                            bool r9_1;
                            if (((*rax_4 <= rbx_1 && rbp_2 == rdx_11) || (*rax_4 > rbx_1 && rdx_11 == r15_1)))
                            {
                                if (rax_5 != rcx_2)
                                {
                                    rdx_2 = r15_1;
                                    if (rdx_2 == rdx_11)
                                    {
                                    label_402296:
                                        int64_t rax_22 = data_60b328;
                                        rbx_1 = (rbx_1 + 1);
                                        if (rbx_1 <= *(rax_22 + 8))
                                        {
                                            goto label_401f94;
                                        }
                                        data_60b328 = (rax_22 + 0x10);
                                        goto label_401f94;
                                    }
                                label_4022d5:
                                    rax_5 = data_60b230;
                                    r8_1 = rdx_2 == 0xffffffff;
                                    goto label_4022e0;
                                }
                                rax_24 = r13[1];
                                if (rax_24 >= r13[2])
                                {
                                    rax_26 = __uflow(r13);
                                    rdx_2 = rax_26;
                                    if (rax_26 == 0xffffffff)
                                    {
                                        rsi_3 = data_60b230;
                                        r9_1 = rbp_1;
                                        rdi_4 = data_60b311;
                                        rcx_3 = 0;
                                        rbp_1 = true;
                                        rax_5 = rsi_3;
                                    }
                                }
                                else
                                {
                                    r13[1] = &rax_24[1];
                                    rdx_2 = *rax_24;
                                }
                                if ((rax_24 < r13[2] || (rax_24 >= r13[2] && rax_26 != 0xffffffff)))
                                {
                                    ungetc(rdx_2, r13);
                                    rcx_3 = data_60b311;
                                    rdx_2 = r15_1;
                                    rdi_4 = rcx_3;
                                }
                            }
                            if ((*rax_4 <= rbx_1 && rbp_2 != rdx_11))
                            {
                                rdx_2 = rax_5;
                                if (rbp_2 == rdx_2)
                                {
                                    rbp_1 = true;
                                }
                            }
                            if ((((*rax_4 <= rbx_1 && rbp_2 != rdx_11) && rbp_2 == rdx_2) || ((*rax_4 > rbx_1 && rdx_11 != r15_1) && r15_1 == rdx_2)))
                            {
                                rcx_3 = rcx_2;
                                rdi_4 = rcx_3;
                            }
                            if ((((((*rax_4 <= rbx_1 && rbp_2 == rdx_11) || (*rax_4 > rbx_1 && rdx_11 == r15_1)) && rax_5 == rcx_2) && (rax_24 < r13[2] || (rax_24 >= r13[2] && rax_26 != 0xffffffff))) || ((((*rax_4 <= rbx_1 && rbp_2 != rdx_11) && rbp_2 == rdx_2) || ((*rax_4 > rbx_1 && rdx_11 != r15_1) && r15_1 == rdx_2)) && data_60b230 != rcx_3)))
                            {
                                if (rdx_2 == rcx_3)
                                {
                                    goto label_402296;
                                }
                                goto label_4022d5;
                            }
                            if (((((*rax_4 <= rbx_1 && rbp_2 != rdx_11) && rbp_2 == rdx_2) || ((*rax_4 > rbx_1 && rdx_11 != r15_1) && r15_1 == rdx_2)) && data_60b230 == rcx_3))
                            {
                                r8_1 = false;
                            label_4022e0:
                                rsi_3 = rax_5;
                                rcx_3 = rsi_3 == rdx_2;
                                if ((r8_1 == 0 && rcx_3 == 0))
                                {
                                    goto label_401f94;
                                }
                                r9_1 = rbp_1;
                                rbp_1 = r8_1;
                            }
                            if (((*rax_4 <= rbx_1 && rbp_2 != rdx_11) && rbp_2 != rdx_2))
                            {
                                if (rbp_2 != 0xffffffff)
                                {
                                    FILE* rdi_5 = stdout;
                                    char* rax_6 = stdout[5];
                                    if (rax_6 >= stdout[6])
                                    {
                                        __overflow(rdi_5, rbp_2);
                                    }
                                    else
                                    {
                                        stdout[5] = &rax_6[1];
                                        *rax_6 = rbp_2;
                                    }
                                    r12 = rbp_2;
                                    goto label_40203f;
                                }
                                rbp_1 = true;
                            }
                            if ((((*rax_4 > rbx_1 && rdx_11 != r15_1) && r15_1 != rdx_2) || ((*rax_4 <= rbx_1 && rbp_2 != rdx_11) && rbp_2 != rdx_2)))
                            {
                                rdi_4 = rcx_2;
                                r9_1 = rbp_1;
                                rsi_3 = rax_5;
                                rdx_2 = -1;
                                rcx_3 = 0;
                                rbp_1 = true;
                            }
                            if ((((((*rax_4 > rbx_1 && rdx_11 != r15_1) && r15_1 != rdx_2) || ((*rax_4 <= rbx_1 && rbp_2 != rdx_11) && rbp_2 != rdx_2)) || (((((*rax_4 <= rbx_1 && rbp_2 == rdx_11) || (*rax_4 > rbx_1 && rdx_11 == r15_1)) && rax_5 == rcx_2) && rax_24 >= r13[2]) && rax_26 == 0xffffffff)) || ((((*rax_4 <= rbx_1 && rbp_2 != rdx_11) && rbp_2 == rdx_2) || ((*rax_4 > rbx_1 && rdx_11 != r15_1) && r15_1 == rdx_2)) && data_60b230 == rcx_3)))
                            {
                                if (r9_1 != 0)
                                {
                                label_4020f7:
                                    rdi_4 = rdi_4 == rax_5;
                                    if (((rsi_3 != r12 | rdi_4) == 0 && rcx_3 == 0))
                                    {
                                        goto label_402230;
                                    }
                                    FILE* rdi_8 = stdout;
                                    char* rcx_4 = stdout[5];
                                    if (rcx_4 >= stdout[6])
                                    {
                                        __overflow(rdi_8, rsi_3);
                                    }
                                    else
                                    {
                                        stdout[5] = &rcx_4[1];
                                        *rcx_4 = rax_5;
                                    }
                                }
                                else
                                {
                                    if (data_60b313 != 1)
                                    {
                                        goto label_4020f7;
                                    }
                                    if (rbx_1 != 1)
                                    {
                                        goto label_4020f7;
                                    }
                                }
                                if (rbp_1 != 0)
                                {
                                    goto label_402230;
                                }
                                data_60b328 = data_60b4e0;
                            label_40213e:
                                rbx_1 = 1;
                                r12 = true;
                                r12 = (1 & var_3d_1);
                                if (r12 != 0)
                                {
                                    break;
                                }
                            }
                        }
                    }
                    void* rax_12 = sub_4032b0(&data_60b320, &data_60b318, nullptr, -1, data_60b311, data_60b230, r13);
                    bool cond:5_1 = rax_12 == 0;
                    if (rax_12 < 0)
                    {
                        free(data_60b320);
                        rax_19 = *r13;
                        data_60b320 = 0;
                        if ((rax_19 & 0x30) == 0)
                        {
                            sub_405bc0();
                            /* no return */
                        }
                        break;
                    }
                    if (cond:5_1)
                    {
                        __assert_fail("n_bytes != 0", "src/cut.c", 0x149, "cut_fields");
                        /* no return */
                    }
                    int64_t rdi_9 = data_60b320;
                    void* rdx_8 = (rax_12 - 1);
                    if (*((rdi_9 + rax_12) - 1) == data_60b311)
                    {
                        rax_4 = data_60b328;
                        if (*rax_4 <= 1)
                        {
                            fwrite_unlocked(rdi_9, 1, rdx_8, stdout);
                            char* rax_28;
                            int32_t rax_29;
                            if (data_60b311 == data_60b230)
                            {
                                rax_28 = r13[1];
                                if (rax_28 >= r13[2])
                                {
                                    rax_29 = __uflow(r13);
                                    if (rax_29 == 0xffffffff)
                                    {
                                        rax_4 = data_60b328;
                                    }
                                }
                                else
                                {
                                    r13[1] = &rax_28[1];
                                    rax_29 = *rax_28;
                                }
                                if ((rax_28 < r13[2] || (rax_28 >= r13[2] && rax_29 != 0xffffffff)))
                                {
                                    ungetc(rax_29, r13);
                                }
                            }
                            if (((data_60b311 == data_60b230 && (rax_28 < r13[2] || (rax_28 >= r13[2] && rax_29 != 0xffffffff))) || data_60b311 != data_60b230))
                            {
                                rax_4 = data_60b328;
                                rbp_1 = r12;
                            }
                        }
                        if (*(rax_4 + 8) > 1)
                        {
                            rbx_1 = 2;
                            r12 = 0;
                            goto label_401fad;
                        }
                        rax_4 = (rax_4 + 0x10);
                        rbx_1 = 2;
                        r12 = 0;
                        data_60b328 = rax_4;
                        goto label_401fad;
                    }
                    if (data_60b313 == 0)
                    {
                        fwrite_unlocked(rdi_9, 1, rax_12, stdout);
                        rdx_2 = data_60b230;
                        if (*((data_60b320 + rax_12) - 1) == rdx_2)
                        {
                            goto label_40213e;
                        }
                        FILE* rdi_10 = stdout;
                        char* rax_17 = stdout[5];
                        if (rax_17 >= stdout[6])
                        {
                            __overflow(rdi_10, rdx_2);
                            rdx_2 = data_60b230;
                            goto label_40213e;
                        }
                        stdout[5] = &rax_17[1];
                        *rax_17 = rdx_2;
                        goto label_40213e;
                    }
                    rbx_1 = 1;
                    rdx_2 = 0;
                }
                goto label_402234;
            }
        }
    label_402230:
        rax_19 = *r13;
    label_402234:
        bool c_1 = false;
        bool z_1 = (rax_19 & 0x20) == 0;
        if (z_1)
        {
            void* const rdi_12 = &data_4094ea;
            int64_t rcx_6 = 2;
            uint64_t rsi_6 = arg1;
            while (rcx_6 != 0)
            {
                char temp4_1 = *rsi_6;
                char temp5_1 = *rdi_12;
                c_1 = temp4_1 < temp5_1;
                z_1 = temp4_1 == temp5_1;
                rsi_6 = (rsi_6 + 1);
                rdi_12 = (rdi_12 + 1);
                rcx_6 = (rcx_6 - 1);
                if ((!z_1))
                {
                    break;
                }
            }
            if ((((!z_1) && (!c_1)) - 0) == 0)
            {
                clearerr_unlocked(r13);
                return 1;
            }
            if (sub_405c20(r13) != 0xffffffff)
            {
                return 1;
            }
        }
    }
    if (((((((!z) && (!c)) - 0) != 0 && rax_30 == 0) || (((!z) && (!c)) - 0) == 0) || ((((!z) && (!c)) - 0) != 0 && rax_30 != 0)))
    {
        error(0, *__errno_location(), "%s", sub_405230(0, 3, arg1));
        return 0;
    }
}

int64_t sub_4026b0(int32_t arg1) __noreturn
{
    int64_t rbp = data_60b350;
    if (arg1 != 0)
    {
        __fprintf_chk(stderr, 1, dcgettext(nullptr, "Try '%s --help' for more informa…", 5), rbp);
    }
    else
    {
        __printf_chk(1, dcgettext(nullptr, "Usage: %s OPTION... [FILE]...\n", 5), rbp);
        fputs_unlocked(dcgettext(nullptr, "Print selected parts of lines fr…", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "\nWith no FILE, or when FILE is …", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "\nMandatory arguments to long op…", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "  -b, --bytes=LIST        select…", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "  -f, --fields=LIST       select…", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "      --complement        comple…", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "  -s, --only-delimited    do not…", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "  -z, --zero-terminated    line …", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "      --help     display this he…", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "      --version  output version …", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "\nUse one, and only one of -b, -…", 5), stdout);
        fputs_unlocked(dcgettext(nullptr, "Each range is one of:\n\n  N    …", 5), stdout);
        void* const var_88 = &data_40755f;
        void* const* rax_16 = &var_88;
        void* const var_78_1 = "coreutils";
        void* const var_70_1 = "Multi-call invocation";
        void* const var_68_1 = "sha224sum";
        void* const var_60_1 = "sha2 utilities";
        void* const var_58_1 = "sha256sum";
        void* const var_50_1 = "sha2 utilities";
        void* const var_48_1 = "sha384sum";
        void* const var_40_1 = "sha2 utilities";
        void* const var_38_1 = "sha512sum";
        void* const var_30_1 = "sha2 utilities";
        int64_t var_28_1 = 0;
        int64_t var_20_1 = 0;
        bool c_1;
        bool z_1;
        do
        {
            rax_16 = &rax_16[2];
            void* const rdi_14 = *rax_16;
            c_1 = false;
            z_1 = (rdi_14 & rdi_14) == 0;
            if (z_1)
            {
                break;
            }
            void* const rsi_14 = &data_40755b;
            int64_t rcx_2 = 4;
            while (rcx_2 != 0)
            {
                char temp0_1 = *rsi_14;
                char temp1_1 = *rdi_14;
                c_1 = temp0_1 < temp1_1;
                z_1 = temp0_1 == temp1_1;
                rsi_14 = (rsi_14 + 1);
                rdi_14 = (rdi_14 + 1);
                rcx_2 = (rcx_2 - 1);
                if ((!z_1))
                {
                    break;
                }
            }
        } while ((((!z_1) && (!c_1)) - 0) != 0);
        void* const rbp_1 = rax_16[1];
        char* rax_27;
        int32_t rax_28;
        void* const r12_1;
        if (rbp_1 == 0)
        {
            int64_t rcx_5;
            int64_t rdx_5;
            uint64_t r8_2;
            ssize_t r9_2;
            rcx_5 = __printf_chk(1, dcgettext(nullptr, "\n%s online help: <%s>\n", 5), "GNU coreutils", "https://www.gnu.org/software/cor…");
            rax_27 = setlocale(5, nullptr, rdx_5, rcx_5, r8_2, r9_2, var_88, "test invocation");
            if (rax_27 != 0)
            {
                rax_28 = strncmp(rax_27, &data_4075e3, 3);
                if (rax_28 != 0)
                {
                    rbp_1 = &data_40755b;
                }
            }
            if ((rax_27 == 0 || (rax_27 != 0 && rax_28 == 0)))
            {
                rbp_1 = &data_40755b;
                r12_1 = " invocation";
                __printf_chk(1, dcgettext(nullptr, "Full documentation at: <%s%s>\n", 5), "https://www.gnu.org/software/cor…", &data_40755b);
            }
        }
        else
        {
            int64_t rcx_3;
            int64_t rdx_3;
            uint64_t r8_1;
            ssize_t r9_1;
            rcx_3 = __printf_chk(1, dcgettext(nullptr, "\n%s online help: <%s>\n", 5), "GNU coreutils", "https://www.gnu.org/software/cor…");
            char* rax_19 = setlocale(5, nullptr, rdx_3, rcx_3, r8_1, r9_1, var_88, "test invocation");
            if (rax_19 == 0)
            {
                goto label_4029d0;
            }
            if (strncmp(rax_19, &data_4075e3, 3) == 0)
            {
                goto label_4029d0;
            }
        }
        if ((((rbp_1 == 0 && rax_27 != 0) && rax_28 != 0) || rbp_1 != 0))
        {
            __printf_chk(1, dcgettext(nullptr, "Report %s translation bugs to <h…", 5), &data_40755b);
        label_4029d0:
            r12_1 = " invocation";
            __printf_chk(1, dcgettext(nullptr, "Full documentation at: <%s%s>\n", 5), "https://www.gnu.org/software/cor…", &data_40755b);
            if (rbp_1 != &data_40755b)
            {
                r12_1 = &data_4094ec[0x1b];
            }
        }
        __printf_chk(1, dcgettext(nullptr, "or available locally via: info '…", 5), rbp_1, r12_1);
    }
    exit(arg1);
    /* no return */
}

uint64_t sub_402ae0(int32_t* arg1, int32_t* arg2)
{
    uint64_t rax = 0xffffffff;
    int32_t rdx = *arg2;
    int32_t temp0 = *arg1;
    bool cond:0 = temp0 > rdx;
    if (temp0 >= rdx)
    {
        rax = cond:0;
        rax = rax;
    }
    return rax;
}

int64_t* sub_402b00(int64_t arg1, int64_t arg2)
{
    int64_t rdx = data_60b4e8;
    int64_t rcx = data_60b338;
    void* rax = data_60b4e0;
    if (rdx == rcx)
    {
        uint64_t rsi_1;
        if (rax != 0)
        {
            if (rcx > 0x555555555555554)
            {
            label_402bb7:
                sub_405bc0();
                /* no return */
            }
            rcx = ((rcx + (rcx >> 1)) + 1);
            rsi_1 = (rcx << 4);
            goto label_402b79;
        }
        if (rcx != 0)
        {
            int64_t rdx_4;
            rdx_4 = (rcx >> 0x3c) != 0;
            rsi_1 = (rcx << 4);
            if (rsi_1 < 0)
            {
                goto label_402bb7;
            }
            if (rdx_4 == 0)
            {
                goto label_402b79;
            }
            goto label_402bb7;
        }
        rsi_1 = 0x80;
        rcx = 8;
    label_402b79:
        data_60b338 = rcx;
        rax = sub_4059d0(rax, rsi_1);
        rdx = data_60b4e8;
        data_60b4e0 = rax;
    }
    int64_t* rax_1 = (rax + (rdx << 4));
    *rax_1 = arg1;
    rax_1[1] = arg2;
    data_60b4e8 = (rdx + 1);
    return rax_1;
}

int64_t* sub_402bd0(char* arg1, int32_t arg2)
{
    char* r15 = arg1;
    bool c = false;
    bool z = (arg2 & 1) == 0;
    char var_3c = arg2;
    int32_t var_40 = (arg2 & 1);
    int32_t rax_1;
    void* rcx;
    char rbx;
    char* rdi_1;
    if ((!z))
    {
        void* const rdi = &data_4094ea;
        rcx = 2;
        char* rsi = r15;
        while (rcx != 0)
        {
            char temp1_1 = *rsi;
            char temp2_1 = *rdi;
            c = temp1_1 < temp2_1;
            z = temp1_1 == temp2_1;
            rsi = &rsi[1];
            rdi = (rdi + 1);
            rcx = (rcx - 1);
            if ((!z))
            {
                break;
            }
        }
        rax_1 = ((!z) && (!c));
        rax_1 = (rax_1 - 0);
        if (rax_1 == 0)
        {
            r15 = &r15[1];
            rdi_1 = 1;
            rbx = 1;
        }
    }
    if ((z || ((!z) && rax_1 != 0)))
    {
        rdi_1 = nullptr;
        rbx = 0;
    }
    char rbp = rbx;
    char r12 = 0;
    char r14 = 0;
    char* var_50 = 1;
    char* rax_16;
    while (true)
    {
        char r13_1 = *r15;
        if (r13_1 == 0x2d)
        {
            if (rbp != 0)
            {
                if ((var_3c & 4) == 0)
                {
                    rax_16 = dcgettext(nullptr, "invalid field range", 5);
                }
                else
                {
                    rax_16 = dcgettext(nullptr, "invalid byte or character range", 5);
                }
                break;
            }
            rcx = rdi_1 == 0;
            rcx = (rcx & rbx);
            r12 = rcx;
            if (rcx == 0)
            {
                if (rbx != 0)
                {
                    var_50 = rdi_1;
                    rbp = rbx;
                    rdi_1 = nullptr;
                }
                else
                {
                    r12 = rbx;
                    rdi_1 = nullptr;
                    rbp = 1;
                    var_50 = 1;
                }
                goto label_402cae;
            }
            if ((var_3c & 4) != 0)
            {
                rax_16 = dcgettext(nullptr, "byte/character positions are num…", 5);
                break;
            }
            goto label_4030af;
        }
        if ((r13_1 != 0x2c && ((*__ctype_b_loc()[r13_1] & 1) == 0 && r13_1 != 0)))
        {
            int32_t rax_9 = r13_1;
            if ((rax_9 - 0x30) > 9)
            {
                int64_t rax_23 = sub_4053b0(r15);
                char* rax_24;
                if ((var_3c & 4) == 0)
                {
                    rax_24 = dcgettext(nullptr, "invalid field value %s", 5);
                }
                else
                {
                    rax_24 = dcgettext(nullptr, "invalid byte/character position …", 5);
                }
                error(0, 0, rax_24, rax_23);
                sub_4026b0(1);
                /* no return */
            }
            if ((r12 == 0 || (r12 != 0 && data_60b330 == 0)))
            {
                data_60b330 = r15;
            }
            bool cond:5_1 = rbp != 0;
            if (rbp == 0)
            {
                rbx = 1;
            }
            if (cond:5_1)
            {
                r14 = rbp;
            }
            char* rax_12;
            if (rdi_1 <= 0x1999999999999999)
            {
                rax_12 = ((rax_9 - 0x30) + (rdi_1 * 0xa));
                if ((rax_12 >= rdi_1 && rax_12 != -1))
                {
                    rdi_1 = rax_12;
                    r12 = 1;
                }
            }
            if (((rdi_1 > 0x1999999999999999 || (rdi_1 <= 0x1999999999999999 && rax_12 < rdi_1)) || ((rdi_1 <= 0x1999999999999999 && rax_12 >= rdi_1) && rax_12 == -1)))
            {
                int64_t rbx_6 = data_60b330;
                strspn(rbx_6, "0123456789");
                char* rax_19 = sub_405c00(rbx_6);
                int64_t rax_20 = sub_4053b0(rax_19);
                char* rax_21;
                if ((var_3c & 4) == 0)
                {
                    rax_21 = dcgettext(nullptr, "field number %s is too large", 5);
                }
                else
                {
                    rax_21 = dcgettext(nullptr, "byte/character offset %s is too …", 5);
                }
                error(0, 0, rax_21, rax_20);
                free(rax_19);
                sub_4026b0(1);
                /* no return */
            }
            goto label_402cae;
        }
        char* rsi_10;
        if (rbp == 0)
        {
            if (rdi_1 == 0)
            {
                if ((var_3c & 4) != 0)
                {
                    rax_16 = dcgettext(rdi_1, "byte/character positions are num…", 5);
                    break;
                }
            label_4030af:
                rax_16 = dcgettext(nullptr, "fields are numbered from 1", 5);
                break;
            }
            sub_402b00(rdi_1, rdi_1);
            if (*r15 == 0)
            {
            label_402d17:
                int64_t rsi_2 = data_60b4e8;
                if (rsi_2 != 0)
                {
                    int64_t r15_1 = 0;
                    qsort(data_60b4e0, rsi_2, 0x10, sub_402ae0);
                    int64_t rbx_1 = data_60b4e8;
                    int64_t r14_1 = data_60b4e0;
                    while (r15_1 < rbx_1)
                    {
                        do
                        {
                            r15_1 = (r15_1 + 1);
                            if (r15_1 >= rbx_1)
                            {
                                break;
                            }
                            int64_t rbp_2 = (r15_1 << 4);
                            int64_t r12_1 = (rbp_2 - 0x10);
                            int64_t rdi_4 = (r14_1 + rbp_2);
                            void* rdx_1 = (r14_1 + r12_1);
                            int64_t rax_5 = *(rdx_1 + 8);
                            if (rax_5 < *rdi_4)
                            {
                                break;
                            }
                            int64_t r13_2 = (rbp_2 + 0x10);
                            do
                            {
                                if (*(rdi_4 + 8) >= rax_5)
                                {
                                    rax_5 = *(rdi_4 + 8);
                                }
                                *(rdx_1 + 8) = rax_5;
                                memmove(rdi_4, (r14_1 + r13_2), (((rbx_1 - r15_1) << 4) - 0x10));
                                r14_1 = data_60b4e0;
                                rbx_1 = (data_60b4e8 - 1);
                                data_60b4e8 = rbx_1;
                                if (rbx_1 <= r15_1)
                                {
                                    break;
                                }
                                rdx_1 = (r14_1 + r12_1);
                                rdi_4 = (r14_1 + rbp_2);
                                rax_5 = *(rdx_1 + 8);
                            } while (*rdi_4 <= rax_5);
                            if (rbx_1 <= r15_1)
                            {
                                break;
                            }
                        } while (r15_1 < rbx_1);
                        if (r15_1 < rbx_1)
                        {
                            break;
                        }
                    }
                    if ((var_3c & 2) != 0)
                    {
                        data_60b4e0 = 0;
                        int64_t rax_13 = *r14_1;
                        data_60b4e8 = 0;
                        data_60b338 = 0;
                        if (rax_13 > 1)
                        {
                            sub_402b00(1, (rax_13 - 1));
                        }
                        int64_t* rbp_3 = (r14_1 + 0x10);
                        int64_t r12_4 = ((rbx_1 << 4) + r14_1);
                        if (rbx_1 > 1)
                        {
                            do
                            {
                                int64_t rsi_7 = *rbp_3;
                                int64_t rdi_7 = (rbp_3[-1] + 1);
                                if (rdi_7 != rsi_7)
                                {
                                    sub_402b00(rdi_7, (rsi_7 - 1));
                                }
                                rbp_3 = &rbp_3[2];
                            } while (r12_4 != rbp_3);
                        }
                        int64_t rax_15 = *((r14_1 + (rbx_1 << 4)) - 8);
                        if (rax_15 != -1)
                        {
                            sub_402b00((rax_15 + 1), -1);
                        }
                        free(r14_1);
                        rbx_1 = data_60b4e8;
                        r14_1 = data_60b4e0;
                    }
                    data_60b4e8 = (rbx_1 + 1);
                    int64_t rax_7 = sub_4059d0(r14_1, ((rbx_1 + 1) << 4));
                    int64_t rdx_3 = data_60b4e8;
                    data_60b4e0 = rax_7;
                    *((rax_7 + (rdx_3 << 4)) - 8) = -1;
                    *((rax_7 + (rdx_3 << 4)) - 0x10) = -1;
                    return ((rax_7 + (rdx_3 << 4)) - 0x10);
                }
                if ((var_3c & 4) != 0)
                {
                    rax_16 = dcgettext(nullptr, "missing list of byte/character p…", 5);
                    break;
                }
                rsi_10 = "missing list of fields";
                goto label_402fd6;
            }
        label_402ca2:
            rdi_1 = nullptr;
            r12 = 0;
            rbp = 0;
            r14 = 0;
            rbx = 0;
        label_402cae:
            r15 = &r15[1];
        }
        else
        {
            if (((rbx != 0 && r14 != 0) || (rbx == 0 && r14 != 0)))
            {
                if (var_50 > rdi_1)
                {
                    rsi_10 = "invalid decreasing range";
                    goto label_402fd6;
                }
                sub_402b00(var_50, rdi_1);
            }
            if ((rbx == 0 && r14 == 0))
            {
                if (var_40 == 0)
                {
                    rsi_10 = "invalid range with no endpoint: …";
                label_402fd6:
                    rax_16 = dcgettext(nullptr, rsi_10, 5);
                    break;
                }
                var_50 = 1;
            }
            if (((rbx != 0 && r14 == 0) || (rbx == 0 && r14 == 0)))
            {
                sub_402b00(var_50, -1);
            }
            if ((((rbx != 0 && r14 == 0) || (rbx == 0 && r14 == 0)) || (((rbx != 0 && r14 != 0) || (rbx == 0 && r14 != 0)) && var_50 <= rdi_1)))
            {
                if (*r15 != 0)
                {
                    goto label_402ca2;
                }
                goto label_402d17;
            }
        }
    }
    error(0, 0, rax_16);
    sub_4026b0(1);
    /* no return */
}

int64_t sub_403170()
{
    int64_t rdi = data_60b4e0;
    data_60b4e8 = 0;
    data_60b338 = 0;
    int64_t rax = free(rdi);
    data_60b4e0 = 0;
    return rax;
}

int64_t sub_4031b0(int64_t arg1)
{
    data_60b348 = arg1;
}

int64_t sub_4031c0(char arg1)
{
    data_60b340 = arg1;
}

void sub_4031d0(void* arg1)
{
    int32_t rax = sub_406cd0(stdout);
    int32_t* rax_1;
    if (rax != 0)
    {
        rax_1 = __errno_location();
        if ((data_60b340 == 0 || (data_60b340 != 0 && *rax_1 != 0x20)))
        {
            char* rax_3 = dcgettext(nullptr, "write error", 5);
            int64_t rdi_2 = data_60b348;
            if (rdi_2 == 0)
            {
                error(0, *rax_1, "%s", rax_3);
            }
            else
            {
                error(0, *rax_1, "%s: %s", sub_405200(rdi_2), rax_3);
            }
        }
    }
    if (((rax == 0 || ((rax != 0 && data_60b340 != 0) && *rax_1 == 0x20)) && sub_406cd0(stderr) == 0))
    {
        return;
    }
    _exit(data_60b240);
    /* no return */
}

int64_t sub_403270(int32_t arg1, int64_t arg2, off64_t arg3, int32_t arg4)
{
    /* tailcall */
    return posix_fadvise(arg1, arg2, arg3, arg4);
}

void sub_403280(FILE* arg1, int32_t arg2)
{
    if (arg1 == 0)
    {
        return;
    }
    /* tailcall */
    return posix_fadvise(fileno(arg1), 0, 0, arg2);
}

void* sub_4032b0(int64_t* arg1, int64_t* arg2, void* arg3, uint64_t arg4, 
    int32_t arg5, int32_t arg6, FILE* arg7)
{
    uint64_t r13;
    uint64_t var_18 = r13;
    void* rax = *arg1;
    uint64_t r12 = *arg2;
    void* var_60 = arg3;
    int32_t var_84 = arg5;
    int32_t var_6c = arg6;
    void* var_78 = rax;
    if (rax == 0)
    {
        r12 = 0x40;
        if (arg4 <= 0x40)
        {
            r12 = arg4;
        }
        rax = malloc(r12);
        var_78 = rax;
        if (rax == 0)
        {
        label_4035c1:
            return -1;
        }
    }
    void* rax_10;
    if (r12 >= var_60)
    {
        void* rbp_2 = (r12 - var_60);
        arg3 = r12 == var_60;
        rax = r12 >= arg4;
        if ((arg3 & rax) == 0)
        {
            if (var_84 == 0xffffffff)
            {
                var_84 = var_6c;
            }
            else
            {
                int32_t rbx_1 = var_6c;
                if (rbx_1 == 0xffffffff)
                {
                    rbx_1 = var_84;
                }
                var_6c = rbx_1;
            }
            void* rbx_3 = (var_78 + var_60);
            while (true)
            {
                off_t var_40;
                char* rax_3 = sub_405ce0(arg7, &var_40);
                uint32_t var_70;
                char* rax_4;
                uint32_t rax_7;
                void* rcx;
                if (rax_3 == 0)
                {
                    rax_4 = arg7->_IO_read_ptr;
                    if (rax_4 >= arg7->_IO_read_end)
                    {
                        rax_7 = __uflow(arg7);
                        var_70 = rax_7;
                        if ((rax_7 == 0xffffffff && rbx_3 == var_78))
                        {
                            break;
                        }
                    }
                    else
                    {
                        arg7->_IO_read_ptr = &rax_4[1];
                        var_70 = *rax_4;
                    }
                    if ((rax_4 < arg7->_IO_read_end || (rax_4 >= arg7->_IO_read_end && rax_7 != 0xffffffff)))
                    {
                        rcx = 2;
                        var_40 = 1;
                        r13 = var_84 == var_70;
                        uint32_t rax_6;
                        rax_6 = var_6c == var_70;
                        r13 = (r13 | rax_6);
                    }
                }
                else
                {
                    off_t r13_1 = var_40;
                    char* rax_16;
                    if (var_84 != 0xffffffff)
                    {
                        rax_16 = sub_403600(rax_3, var_84, var_6c, r13_1);
                        if (rax_16 != 0)
                        {
                            void* rax_17 = (rax_16 - rax_3);
                            r13 = 1;
                            rcx = (rax_17 + 2);
                            var_40 = (rax_17 + 1);
                        }
                    }
                    if ((var_84 == 0xffffffff || (var_84 != 0xffffffff && rax_16 == 0)))
                    {
                        rcx = (r13_1 + 1);
                        r13 = 0;
                    }
                }
                if (((rax_3 == 0 && (rax_4 < arg7->_IO_read_end || (rax_4 >= arg7->_IO_read_end && rax_7 != 0xffffffff))) || rax_3 != 0))
                {
                    if ((r12 < arg4 && rbp_2 < rcx))
                    {
                        uint64_t rax_12 = (r12 + 0x40);
                        uint64_t rdx_2 = (r12 + r12);
                        if (r12 <= 0x3f)
                        {
                            rdx_2 = rax_12;
                        }
                        void* rbx_5 = (rbx_3 - var_78);
                        void* rax_13 = (rbx_5 + rcx);
                        if ((rdx_2 - rbx_5) < rcx)
                        {
                            rdx_2 = rax_13;
                        }
                        if ((r12 >= rdx_2 || (r12 < rdx_2 && rdx_2 > arg4)))
                        {
                            rdx_2 = arg4;
                        }
                        if ((rdx_2 - var_60) < 0)
                        {
                            rdx_2 = (-0x8000000000000000 + var_60);
                            if (r12 == rdx_2)
                            {
                                break;
                            }
                        }
                        rbp_2 = (rdx_2 - rbx_5);
                        void* rax_15 = realloc(var_78, rdx_2);
                        if (rax_15 == 0)
                        {
                            break;
                        }
                        rbx_3 = (rbx_5 + rax_15);
                        var_78 = rax_15;
                        r12 = rdx_2;
                    }
                    if (rbp_2 > 1)
                    {
                        off_t rcx_1 = (rbp_2 - 1);
                        if (var_40 <= rcx_1)
                        {
                            rcx_1 = var_40;
                        }
                        if (rax_3 == 0)
                        {
                            *rbx_3 = var_70;
                        }
                        else
                        {
                            memcpy(rbx_3, rax_3, rcx_1);
                        }
                        rbx_3 = (rbx_3 + rcx_1);
                        rbp_2 = (rbp_2 - rcx_1);
                    }
                    if ((rax_3 != 0 && sub_406d30(arg7, var_40) != 0))
                    {
                        break;
                    }
                    if (r13 == 0)
                    {
                        continue;
                    }
                }
                *rbx_3 = 0;
                void* rax_19 = (var_60 + var_78);
                *arg1 = var_78;
                rax_10 = (rbx_3 - rax_19);
                *arg2 = r12;
                if (rbx_3 == rax_19)
                {
                    goto label_4035c1;
                }
                goto label_403482;
            }
        }
    }
    *arg1 = var_78;
    *arg2 = r12;
    rax_10 = ffffffffffffffff;
label_403482:
    return rax_10;
}

char* sub_403600(char* arg1, int32_t arg2, int32_t arg3, int64_t arg4)
{
    if (arg2 == arg3)
    {
        /* tailcall */
        return sub_406bc0(arg1, arg2, arg4);
    }
    char r10 = arg2;
    char r9 = arg3;
    if ((arg4 != 0 && (arg1 & 7) != 0))
    {
        char rax_1 = *arg1;
        if ((rax_1 == arg3 || (rax_1 != arg3 && rax_1 == arg2)))
        {
            return arg1;
        }
        if ((rax_1 != arg3 && rax_1 != arg2))
        {
            while (true)
            {
                arg1 = &arg1[1];
                int64_t temp0_1 = arg4;
                arg4 = (arg4 - 1);
                if (temp0_1 == 1)
                {
                    break;
                }
                if ((arg1 & 7) == 0)
                {
                    break;
                }
                char rax_2 = *arg1;
                if (rax_2 == r9)
                {
                    return arg1;
                }
                if (rax_2 == r10)
                {
                    return arg1;
                }
            }
        }
    }
    int64_t rax_7 = ((arg3 << 8) | arg3);
    int64_t r8_4 = ((arg2 << 8) | arg2);
    int64_t rax_8 = (rax_7 | (rax_7 << 0x10));
    int64_t r8_5 = (r8_4 | (r8_4 << 0x10));
    int64_t rbp_5 = ((rax_8 << 0x20) | rax_8);
    int64_t rbx_5 = ((r8_5 << 0x20) | r8_5);
    int64_t rax_10;
    void* rax_20;
    int64_t r12_2;
    if (arg4 > 7)
    {
        int64_t rax_9 = *arg1;
        r12_2 = (rbx_5 ^ rax_9);
        rax_10 = (rax_9 ^ rbp_5);
        if (((((!rax_10) & (rax_10 - 0x101010101010101)) | ((r12_2 - 0x101010101010101) & (!r12_2))) & 0x8080808080808080) != 0)
        {
        label_40372c:
            char rax_19 = *arg1;
            if (rax_19 != arg3)
            {
                if (rax_19 == arg2)
                {
                    goto label_403784;
                }
                void* rdx = &arg1[1];
                void* rdi = &arg1[arg4];
                while (true)
                {
                    rax_20 = rdx;
                    if (rdx == rdi)
                    {
                        break;
                    }
                    char rsi = *rdx;
                    rdx = (rdx + 1);
                    if (rsi == r10)
                    {
                        return rax_20;
                    }
                    if (rsi == r9)
                    {
                        return rax_20;
                    }
                }
                goto label_403761;
            }
        label_403784:
            rax_20 = arg1;
        }
        else
        {
            int64_t rax_15;
            int64_t r13_2;
            do
            {
                arg4 = (arg4 - 8);
                arg1 = &arg1[8];
                if (arg4 <= 7)
                {
                    break;
                }
                int64_t rax_14 = *arg1;
                rax_15 = (rax_14 ^ rbp_5);
                r13_2 = (rax_14 ^ rbx_5);
            } while (((((!rax_15) & (rax_15 - 0x101010101010101)) | ((r13_2 - 0x101010101010101) & (!r13_2))) & 0x8080808080808080) == 0);
            if (arg4 > 7)
            {
                goto label_40372c;
            }
        }
    }
    if (((arg4 <= 7 || (arg4 > 7 && ((((!rax_10) & (rax_10 - 0x101010101010101)) | ((r12_2 - 0x101010101010101) & (!r12_2))) & 0x8080808080808080) == 0)) && arg4 == 0))
    {
    label_403761:
        rax_20 = nullptr;
    }
    if ((((arg4 <= 7 || (arg4 > 7 && ((((!rax_10) & (rax_10 - 0x101010101010101)) | ((r12_2 - 0x101010101010101) & (!r12_2))) & 0x8080808080808080) == 0)) && arg4 == 0) || (arg4 > 7 && ((((!rax_10) & (rax_10 - 0x101010101010101)) | ((r12_2 - 0x101010101010101) & (!r12_2))) & 0x8080808080808080) != 0)))
    {
        return rax_20;
    }
    if (((arg4 <= 7 || (arg4 > 7 && ((((!rax_10) & (rax_10 - 0x101010101010101)) | ((r12_2 - 0x101010101010101) & (!r12_2))) & 0x8080808080808080) == 0)) && arg4 != 0))
    {
        goto label_40372c;
    }
}

char* sub_403790(char* arg1)
{
    if (arg1 == 0)
    {
        fwrite("A NULL argv[0] was passed throug…", 1, 0x37, stderr);
        abort();
        /* no return */
    }
    void* rbx = arg1;
    char* rax = strrchr(arg1, 0x2f);
    if (rax != 0)
    {
        void* r8_1 = &rax[1];
        int64_t rdx_2 = (r8_1 - rbx);
        bool c_1 = rdx_2 < 6;
        bool z_1 = rdx_2 == 6;
        if ((!(z_1 || (rdx_2 - 6) < 0 != (rdx_2 - 6))))
        {
            void* rsi_1 = &rax[-6];
            void* const rdi = "/.libs/";
            int64_t rcx_1 = 7;
            while (rcx_1 != 0)
            {
                char temp0_1 = *rsi_1;
                char temp1_1 = *rdi;
                c_1 = temp0_1 < temp1_1;
                z_1 = temp0_1 == temp1_1;
                rsi_1 = (rsi_1 + 1);
                rdi = (rdi + 1);
                rcx_1 = (rcx_1 - 1);
                if ((!z_1))
                {
                    break;
                }
            }
            rdx_2 = ((!z_1) && (!c_1));
            rdx_2 = (rdx_2 - 0);
            bool c_2 = false;
            bool z_2 = (rdx_2 & rdx_2) == 0;
            if (z_2)
            {
                void* const rdi_1 = &data_4082f0;
                int64_t rcx_2 = 3;
                void* rsi_2 = r8_1;
                rbx = r8_1;
                while (rcx_2 != 0)
                {
                    char temp2_1 = *rsi_2;
                    char temp3_1 = *rdi_1;
                    c_2 = temp2_1 < temp3_1;
                    z_2 = temp2_1 == temp3_1;
                    rsi_2 = (rsi_2 + 1);
                    rdi_1 = (rdi_1 + 1);
                    rcx_2 = (rcx_2 - 1);
                    if ((!z_2))
                    {
                        break;
                    }
                }
                rdx_2 = ((!z_2) && (!c_2));
                rdx_2 = (rdx_2 - 0);
                if (rdx_2 == 0)
                {
                    rbx = &rax[4];
                    program_invocation_short_name = rbx;
                }
            }
        }
    }
    data_60b350 = rbx;
    __progname_full = rbx;
    return rax;
}

char* sub_403830(char* arg1, int32_t arg2)
{
    char* rax = dcgettext(nullptr, arg1, 5);
    char* rbx = rax;
    if (arg1 == rax)
    {
        char* rax_2 = sub_406ee0();
        char rdx_2 = (*rax_2 & 0xdf);
        if (rdx_2 != 0x55)
        {
            if (rdx_2 != 0x47)
            {
            label_403911:
                void* const rbx_1 = &data_4082f4;
                if (arg2 != 9)
                {
                    rbx_1 = "'";
                }
                return rbx_1;
            }
            if ((rax_2[1] & 0xdf) != 0x42)
            {
                goto label_403911;
            }
            if (rax_2[2] != 0x31)
            {
                goto label_403911;
            }
            if (rax_2[3] != 0x38)
            {
                goto label_403911;
            }
            if (rax_2[4] != 0x30)
            {
                goto label_403911;
            }
            if (rax_2[5] != 0x33)
            {
                goto label_403911;
            }
            if (rax_2[6] != 0x30)
            {
                goto label_403911;
            }
            if (rax_2[7] != 0)
            {
                goto label_403911;
            }
            bool cond:2_1 = *rbx != 0x60;
            rbx = &data_4082fd;
            if (cond:2_1)
            {
                rbx = &data_4082fa;
            }
        }
        else
        {
            if ((rax_2[1] & 0xdf) != 0x54)
            {
                goto label_403911;
            }
            if ((rax_2[2] & 0xdf) != 0x46)
            {
                goto label_403911;
            }
            if (rax_2[3] != 0x2d)
            {
                goto label_403911;
            }
            if (rax_2[4] != 0x38)
            {
                goto label_403911;
            }
            if (rax_2[5] != 0)
            {
                goto label_403911;
            }
            bool cond:1_1 = *rbx != 0x60;
            rbx = &data_408301;
            if (cond:1_1)
            {
                rbx = &data_4082f6;
            }
        }
    }
    return rbx;
}

int64_t sub_403930(char* arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, 
    uint32_t arg5, int32_t arg6, int64_t arg7, char* arg8, char* arg9)
{
    char* r15 = arg1;
    uint64_t r14 = arg2;
    uint32_t r13 = arg5;
    uint64_t rax;
    uint64_t rdx;
    rax = __ctype_get_mb_cur_max();
    int32_t rbp_1 = (arg6 & 2);
    bool var_5c = rbp_1 != 0;
    bool cond:1 = r13 == 0xa;
    if (r13 > 0xa)
    {
        abort();
        /* no return */
    }
    uint64_t r11 = arg4;
    uint64_t var_c0;
    char var_a9;
    uint64_t var_a0_1;
    char const* const var_78_1;
    uint64_t var_70;
    char var_5b_1;
    uint64_t rax_1;
    int64_t rbx_1;
    uint32_t r10;
    if ((r13 == 1 || (r13 == 4 && rbp_1 != 0)))
    {
        var_a9 = 1;
        r10 = 0;
        rbx_1 = 0;
        r13 = 2;
        var_5b_1 = 0;
        var_70 = 0;
        var_5c = true;
        var_c0 = 0;
        var_a0_1 = 1;
        var_78_1 = "'";
    label_4039c8:
        while (true)
        {
            uint64_t r9 = r11;
            int64_t r12_1 = 0;
            uint32_t r11_1 = r13;
            while (true)
            {
                r13 = r9 != r12_1;
                if (r9 == -1)
                {
                    r13 = *(arg3 + r12_1) != 0;
                }
                uint32_t r13_1;
                if (r13 == 0)
                {
                    r11 = r9;
                    rdx = r11_1 == 2;
                    if ((rbx_1 == 0 && (var_5c & rdx) != 0))
                    {
                        r13_1 = 2;
                    label_403d3c:
                        if (var_c0 != 0)
                        {
                            r13_1 = 4;
                        }
                        goto label_403d84;
                    }
                    rax_1 = (var_5c ^ 1);
                    rdx = (rdx & rax_1);
                    if (rdx == 0)
                    {
                        rdx = rax_1;
                    }
                    else if (var_5b_1 != 0)
                    {
                        if (var_a9 != 0)
                        {
                            rbx_1 = sub_403930(r15, var_70, arg3, r11, 5, arg6, arg7, arg8, arg9);
                        }
                        else
                        {
                            rax_1 = r14 == 0;
                            rdx = var_70 != 0;
                            rax_1 = (rax_1 & rdx);
                            if (rax_1 != 0)
                            {
                                r14 = var_70;
                                break;
                            }
                            rdx = var_5b_1;
                        }
                    }
                    if (((rdx == 0 || (rdx != 0 && var_5b_1 == 0)) || ((rdx != 0 && var_5b_1 != 0) && var_a9 == 0)))
                    {
                        if ((var_78_1 != 0 && rdx != 0))
                        {
                            char const rdx_17 = *var_78_1;
                            if (rdx_17 != 0)
                            {
                                void* rax_51 = (var_78_1 - rbx_1);
                                do
                                {
                                    if (r14 > rbx_1)
                                    {
                                        r15[rbx_1] = rdx_17;
                                    }
                                    rbx_1 = (rbx_1 + 1);
                                    rdx_17 = *(rax_51 + rbx_1);
                                } while (rdx_17 != 0);
                            }
                        }
                        if (r14 > rbx_1)
                        {
                            r15[rbx_1] = 0;
                        }
                    }
                    goto label_403d9b;
                }
                rax_1 = r11_1 != 2;
                rax_1 = (rax_1 & var_c0);
                char* r8 = (arg3 + r12_1);
                uint64_t var_b8;
                var_b8 = rax_1;
                uint32_t rax_8;
                uint64_t rax_12;
                char rax_14;
                int32_t rcx_2;
                uint64_t rbp_3;
                if (rax_1 == 0)
                {
                    rbp_3 = *r8;
                    if (rbp_3 > 0x7e)
                    {
                        goto label_403ed5;
                    }
                    switch (rbp_3)
                    {
                        case 0:
                        {
                            if (var_c0 == 0)
                            {
                                if ((arg6 & 1) != 0)
                                {
                                    r12_1 = (r12_1 + 1);
                                    continue;
                                }
                                else
                                {
                                    var_b8 = 0;
                                    rdx = r11_1 == 2;
                                    r13 = 0;
                                    rbp_3 = 0;
                                }
                                goto label_403b25;
                            }
                            goto label_403e10;
                        }
                        case 1:
                        case 2:
                        case 3:
                        case 4:
                        case 5:
                        case 6:
                        case 0xe:
                        case 0xf:
                        case 0x10:
                        case 0x11:
                        case 0x12:
                        case 0x13:
                        case 0x14:
                        case 0x15:
                        case 0x16:
                        case 0x17:
                        case 0x18:
                        case 0x19:
                        case 0x1a:
                        case 0x1b:
                        case 0x1c:
                        case 0x1d:
                        case 0x1e:
                        case 0x1f:
                        case 0x40:
                        {
                            goto label_403ed5;
                        }
                        case 7:
                        {
                            rbp_3 = 7;
                            rcx_2 = 0x61;
                            rdx = r11_1 == 2;
                            goto label_403d00;
                        }
                        case 8:
                        {
                            rbp_3 = 8;
                            rcx_2 = 0x62;
                            rdx = r11_1 == 2;
                        label_403d00:
                            if (var_c0 == 0)
                            {
                                goto label_403d0b;
                            }
                            rbp_3 = rcx_2;
                            r13 = 0;
                        label_403b67:
                            if (var_5c == 0)
                            {
                                goto label_403b78;
                            }
                        label_403d30:
                            r13_1 = r11_1;
                            rax_14 = rdx;
                            r11 = r9;
                            goto label_403d38;
                        }
                        case 9:
                        {
                            rbp_3 = 9;
                            rcx_2 = 0x74;
                            goto label_403f6b;
                        }
                        case 0xa:
                        {
                            rbp_3 = 0xa;
                            rcx_2 = 0x6e;
                        label_403f6b:
                            rdx = r11_1 == 2;
                            bool rax_26 = (var_5c & rdx);
                            var_b8 = rax_26;
                            if (rax_26 != 0)
                            {
                            label_403f7a:
                                r11 = r9;
                                r13_1 = 2;
                                goto label_403d3c;
                            }
                            goto label_403d00;
                        }
                        case 0xb:
                        {
                            rbp_3 = 0xb;
                            rcx_2 = 0x76;
                            rdx = r11_1 == 2;
                            goto label_403d00;
                        }
                        case 0xc:
                        {
                            rbp_3 = 0xc;
                            rcx_2 = 0x66;
                            rdx = r11_1 == 2;
                            goto label_403d00;
                        }
                        case 0xd:
                        {
                            rbp_3 = 0xd;
                            rcx_2 = 0x72;
                            goto label_403f6b;
                        }
                        case 0x20:
                        {
                            rbp_3 = 0x20;
                            rdx = r11_1 == 2;
                            goto label_403b06;
                        }
                        case 0x21:
                        case 0x22:
                        case 0x24:
                        case 0x26:
                        case 0x28:
                        case 0x29:
                        case 0x2a:
                        case 0x3b:
                        case 0x3c:
                        case 0x3d:
                        case 0x3e:
                        case 0x5b:
                        case 0x5e:
                        case 0x60:
                        case 0x7c:
                        {
                            rdx = r11_1 == 2;
                            r13 = 0;
                        label_403b06:
                            if (var_5c == 0)
                            {
                                goto label_403b25;
                            }
                            if (rdx == 0)
                            {
                                goto label_403b25;
                            }
                            goto label_403f7a;
                        }
                        case 0x23:
                        case 0x7e:
                        {
                            rdx = r11_1 == 2;
                        label_403afd:
                            if (r12_1 != 0)
                            {
                                goto label_403d0b;
                            }
                            goto label_403b06;
                        }
                        case 0x25:
                        case 0x2b:
                        case 0x2c:
                        case 0x2d:
                        case 0x2e:
                        case 0x2f:
                        case 0x30:
                        case 0x31:
                        case 0x32:
                        case 0x33:
                        case 0x34:
                        case 0x35:
                        case 0x36:
                        case 0x37:
                        case 0x38:
                        case 0x39:
                        case 0x3a:
                        case 0x41:
                        case 0x42:
                        case 0x43:
                        case 0x44:
                        case 0x45:
                        case 0x46:
                        case 0x47:
                        case 0x48:
                        case 0x49:
                        case 0x4a:
                        case 0x4b:
                        case 0x4c:
                        case 0x4d:
                        case 0x4e:
                        case 0x4f:
                        case 0x50:
                        case 0x51:
                        case 0x52:
                        case 0x53:
                        case 0x54:
                        case 0x55:
                        case 0x56:
                        case 0x57:
                        case 0x58:
                        case 0x59:
                        case 0x5a:
                        case 0x5d:
                        case 0x5f:
                        case 0x61:
                        case 0x62:
                        case 0x63:
                        case 0x64:
                        case 0x65:
                        case 0x66:
                        case 0x67:
                        case 0x68:
                        case 0x69:
                        case 0x6a:
                        case 0x6b:
                        case 0x6c:
                        case 0x6d:
                        case 0x6e:
                        case 0x6f:
                        case 0x70:
                        case 0x71:
                        case 0x72:
                        case 0x73:
                        case 0x74:
                        case 0x75:
                        case 0x76:
                        case 0x77:
                        case 0x78:
                        case 0x79:
                        case 0x7a:
                        {
                        label_403f30:
                            rdx = r11_1 == 2;
                        label_403b25:
                            rax_8 = ((((var_c0 ^ 1) | rdx) ^ 1) | var_5c);
                            if (rax_8 == 0)
                            {
                                goto label_403c2c;
                            }
                            goto label_403b2f;
                        }
                        case 0x27:
                        {
                            goto label_403cad;
                        }
                        case 0x3f:
                        {
                            goto label_403c7d;
                        }
                        case 0x5c:
                        {
                            if (r11_1 != 2)
                            {
                                goto label_403cda;
                            }
                            if (var_5c != 0)
                            {
                                goto label_404a6d;
                            }
                            r12_1 = (r12_1 + 1);
                            rax_12 = r10;
                            r13 = 0;
                            rbp_3 = 0x5c;
                            goto label_403c44;
                        }
                        case 0x7b:
                        case 0x7d:
                        {
                            goto label_403ad9;
                        }
                    }
                }
                else
                {
                    char var_98;
                    uint64_t var_90;
                    uint32_t var_88;
                    char rax_27;
                    char rax_28;
                    if (var_a0_1 != 0)
                    {
                        int64_t rbp_2 = (r12_1 + var_a0_1);
                        if ((r9 == -1 && var_a0_1 > 1))
                        {
                            var_90 = r10;
                            var_98 = r8;
                            uint64_t rax_3;
                            rax_3 = strlen(arg3);
                            r10 = var_90;
                            r8 = var_98;
                            r9 = rax_3;
                        }
                        if (rbp_2 > r9)
                        {
                            goto label_4040d0;
                        }
                        uint64_t var_80;
                        var_80 = r11_1;
                        var_88 = r9;
                        var_90 = r10;
                        var_98 = r8;
                        int32_t rax_4;
                        rax_4 = memcmp(r8, var_78_1, var_a0_1);
                        r8 = var_98;
                        r10 = var_90;
                        r9 = var_88;
                        r11_1 = var_80;
                        if (rax_4 != 0)
                        {
                            goto label_4040d0;
                        }
                        if (var_5c != 0)
                        {
                        label_404a20:
                            r13_1 = r11_1;
                            r11 = r9;
                        label_403d84:
                            rbx_1 = sub_403930(r15, r14, arg3, r11, r13_1, (arg6 & 0xfffffffd), 0, arg8, arg9);
                        label_403d9b:
                            return rbx_1;
                        }
                        rbp_3 = *r8;
                        if (rbp_3 > 0x7e)
                        {
                            goto label_403ed5;
                        }
                        switch (rbp_3)
                        {
                            case 0:
                            {
                                goto label_403e27;
                            }
                            case 1:
                            case 2:
                            case 3:
                            case 4:
                            case 5:
                            case 6:
                            case 0xe:
                            case 0xf:
                            case 0x10:
                            case 0x11:
                            case 0x12:
                            case 0x13:
                            case 0x14:
                            case 0x15:
                            case 0x16:
                            case 0x17:
                            case 0x18:
                            case 0x19:
                            case 0x1a:
                            case 0x1b:
                            case 0x1c:
                            case 0x1d:
                            case 0x1e:
                            case 0x1f:
                            case 0x40:
                            {
                                goto label_403ed5;
                            }
                            case 7:
                            {
                                rbp_3 = 0x61;
                            label_403df0:
                                r13 = 0;
                                goto label_403bab;
                            }
                            case 8:
                            {
                                rdx = 0;
                                r13 = 0;
                                rbp_3 = 0x62;
                            label_403b78:
                                uint32_t rax_10;
                                rax_10 = ((r10 ^ 1) & rdx);
                                if (rax_10 != 0)
                                {
                                    if (r14 > rbx_1)
                                    {
                                        r15[rbx_1] = 0x27;
                                    }
                                    if ((rbx_1 + 1) < r14)
                                    {
                                        r15[(rbx_1 + 1)] = 0x24;
                                    }
                                    if ((rbx_1 + 2) < r14)
                                    {
                                        r15[(rbx_1 + 2)] = 0x27;
                                    }
                                    rbx_1 = (rbx_1 + 3);
                                    r10 = rax_10;
                                }
                            label_403bab:
                                if (r14 <= rbx_1)
                                {
                                    goto label_403bb5;
                                }
                                r15[rbx_1] = 0x5c;
                            label_403bb5:
                                rbx_1 = (rbx_1 + 1);
                                r12_1 = (r12_1 + 1);
                                goto label_403bbd;
                            }
                            case 9:
                            {
                                goto label_403da0;
                            }
                            case 0xa:
                            {
                                goto label_403d18;
                            }
                            case 0xb:
                            {
                                goto label_403e00;
                            }
                            case 0xc:
                            {
                                goto label_403de0;
                            }
                            case 0xd:
                            {
                                goto label_403dc9;
                            }
                            case 0x20:
                            {
                                rax_27 = var_b8;
                                goto label_40412a;
                            }
                            case 0x21:
                            case 0x22:
                            case 0x24:
                            case 0x26:
                            case 0x28:
                            case 0x29:
                            case 0x2a:
                            case 0x3b:
                            case 0x3c:
                            case 0x3d:
                            case 0x3e:
                            case 0x5b:
                            case 0x5e:
                            case 0x60:
                            case 0x7c:
                            {
                                goto label_403db5;
                            }
                            case 0x23:
                            case 0x7e:
                            {
                                rax_28 = var_b8;
                                goto label_40410a;
                            }
                            case 0x25:
                            case 0x2b:
                            case 0x2c:
                            case 0x2d:
                            case 0x2e:
                            case 0x2f:
                            case 0x30:
                            case 0x31:
                            case 0x32:
                            case 0x33:
                            case 0x34:
                            case 0x35:
                            case 0x36:
                            case 0x37:
                            case 0x38:
                            case 0x39:
                            case 0x3a:
                            case 0x41:
                            case 0x42:
                            case 0x43:
                            case 0x44:
                            case 0x45:
                            case 0x46:
                            case 0x47:
                            case 0x48:
                            case 0x49:
                            case 0x4a:
                            case 0x4b:
                            case 0x4c:
                            case 0x4d:
                            case 0x4e:
                            case 0x4f:
                            case 0x50:
                            case 0x51:
                            case 0x52:
                            case 0x53:
                            case 0x54:
                            case 0x55:
                            case 0x56:
                            case 0x57:
                            case 0x58:
                            case 0x59:
                            case 0x5a:
                            case 0x5d:
                            case 0x5f:
                            case 0x61:
                            case 0x62:
                            case 0x63:
                            case 0x64:
                            case 0x65:
                            case 0x66:
                            case 0x67:
                            case 0x68:
                            case 0x69:
                            case 0x6a:
                            case 0x6b:
                            case 0x6c:
                            case 0x6d:
                            case 0x6e:
                            case 0x6f:
                            case 0x70:
                            case 0x71:
                            case 0x72:
                            case 0x73:
                            case 0x74:
                            case 0x75:
                            case 0x76:
                            case 0x77:
                            case 0x78:
                            case 0x79:
                            case 0x7a:
                            {
                                rax_27 = var_b8;
                                goto label_4040f2;
                            }
                            case 0x27:
                            {
                                goto label_403cad;
                            }
                            case 0x3f:
                            {
                                goto label_403c7d;
                            }
                            case 0x5c:
                            {
                                goto label_403cda;
                            }
                            case 0x7b:
                            case 0x7d:
                            {
                                goto label_403ad9;
                            }
                        }
                    }
                    else
                    {
                    label_4040d0:
                        rbp_3 = *r8;
                        if (rbp_3 > 0x7e)
                        {
                        label_403ed0:
                            var_b8 = 0;
                        label_403ed5:
                            uint64_t rdi_4;
                            if (rax == 1)
                            {
                                r10 = r10;
                                rdi_4 = 1;
                                rax_12 = (*__ctype_b_loc()[rbp_3] & 0x4000);
                                r13 = rax_12 != 0;
                                rdx = rax_12 == 0;
                                rdx = (rdx & var_c0);
                            label_403f24:
                                if (rdx == 0)
                                {
                                    goto label_403f30;
                                }
                                rdx = var_c0;
                                r13 = 0;
                            label_404421:
                                var_98 = r13;
                                int32_t rsi_7 = 0;
                                int64_t rcx_11 = (rdi_4 + r12_1);
                                char rdi_9 = var_b8;
                                while (true)
                                {
                                    if (rdx != 0)
                                    {
                                        rax_12 = r11_1 == 2;
                                        if (var_5c != 0)
                                        {
                                            break;
                                        }
                                        rax_12 = (rax_12 & (r10 ^ 1));
                                        if (rax_12 != 0)
                                        {
                                            if (r14 > rbx_1)
                                            {
                                                r15[rbx_1] = 0x27;
                                            }
                                            if (r14 > (rbx_1 + 1))
                                            {
                                                r15[(rbx_1 + 1)] = 0x24;
                                            }
                                            if (r14 > (rbx_1 + 2))
                                            {
                                                r15[(rbx_1 + 2)] = 0x27;
                                            }
                                            rbx_1 = (rbx_1 + 3);
                                            r10 = rax_12;
                                        }
                                        if (r14 > rbx_1)
                                        {
                                            r15[rbx_1] = 0x5c;
                                        }
                                        if (r14 > (rbx_1 + 1))
                                        {
                                            r15[(rbx_1 + 1)] = ((rbp_3 >> 6) + 0x30);
                                        }
                                        if (r14 > (rbx_1 + 2))
                                        {
                                            int32_t rax_40;
                                            rax_40 = (rbp_3 >> 3);
                                            r15[(rbx_1 + 2)] = ((rax_40 & 7) + 0x30);
                                        }
                                        r12_1 = (r12_1 + 1);
                                        rbx_1 = (rbx_1 + 3);
                                        rbp_3 = ((rbp_3 & 7) + 0x30);
                                        if (r12_1 >= rcx_11)
                                        {
                                            break;
                                        }
                                        rsi_7 = rdx;
                                    }
                                    else
                                    {
                                        rax_12 = ((rsi_7 ^ 1) & r10);
                                        if (rdi_9 != 0)
                                        {
                                            if (r14 > rbx_1)
                                            {
                                                r15[rbx_1] = 0x5c;
                                            }
                                            rbx_1 = (rbx_1 + 1);
                                        }
                                        r12_1 = (r12_1 + 1);
                                        if (r12_1 >= rcx_11)
                                        {
                                            break;
                                        }
                                        if (rax_12 == 0)
                                        {
                                            rdi_9 = 0;
                                        }
                                        else
                                        {
                                            if (r14 > rbx_1)
                                            {
                                                r15[rbx_1] = 0x27;
                                            }
                                            if (r14 > (rbx_1 + 1))
                                            {
                                                r15[(rbx_1 + 1)] = 0x27;
                                            }
                                            rbx_1 = (rbx_1 + 2);
                                            rdi_9 = 0;
                                            r10 = 0;
                                        }
                                    }
                                    if (r14 > rbx_1)
                                    {
                                        r15[rbx_1] = rbp_3;
                                    }
                                    rbp_3 = *(arg3 + r12_1);
                                    rbx_1 = (rbx_1 + 1);
                                }
                                if ((rdx == 0 && r12_1 >= rcx_11))
                                {
                                    r13 = var_98;
                                    goto label_403c44;
                                }
                                if ((rdx != 0 && var_5c != 0))
                                {
                                    r13_1 = r11_1;
                                    var_c0 = rax_12;
                                    r11 = r9;
                                    goto label_403d3c;
                                }
                                r13 = var_98;
                                goto label_403bbd;
                            }
                            mbstate_t var_40;
                            var_40.__count = 0;
                            var_40.__value = 0;
                            if (r9 == -1)
                            {
                                var_90 = r10;
                                var_98 = r8;
                                r10 = var_90;
                                r8 = var_98;
                                r9 = strlen(arg3);
                            }
                            uint64_t rbx_2 = 0;
                            char var_5a_1 = r10;
                            char var_59_1 = rbp_3;
                            var_88 = r15;
                            var_98 = r9;
                            var_90 = r11_1;
                            while (true)
                            {
                                int64_t r15_1 = (r12_1 + rbx_2);
                                char* rbp_4 = (arg3 + r15_1);
                                uint32_t var_44;
                                rax_12 = sub_406b40(&var_44, rbp_4, (var_98 - r15_1), &var_40);
                                if (rax_12 != 0)
                                {
                                    if (rax_12 == -1)
                                    {
                                        rdi_4 = rbx_2;
                                        rbp_3 = var_59_1;
                                        r13 = 0;
                                        r10 = var_5a_1;
                                        r15 = var_88;
                                        r9 = var_98;
                                        r11_1 = var_90;
                                        rdx = var_c0;
                                        break;
                                    }
                                    if (rax_12 == -2)
                                    {
                                        r9 = var_98;
                                        rax_12 = rbx_2;
                                        rdi_4 = rbx_2;
                                        r10 = var_5a_1;
                                        rbp_3 = var_59_1;
                                        r15 = var_88;
                                        r11_1 = var_90;
                                        if ((r15_1 < r9 && *rbp_4 != 0))
                                        {
                                            do
                                            {
                                                rax_12 = (rax_12 + 1);
                                                if ((r12_1 + rax_12) >= r9)
                                                {
                                                    break;
                                                }
                                            } while (r8[rax_12] != 0);
                                            rdi_4 = rax_12;
                                        }
                                        rdx = var_c0;
                                        r13 = 0;
                                        break;
                                    }
                                    if ((var_90 == 2 && (var_5c != 0 && rax_12 != 1)))
                                    {
                                        char* rax_34 = ((arg3 + r15_1) + 1);
                                        int64_t rsi_6 = ((arg3 + rax_12) + r15_1);
                                        char rcx_9;
                                        do
                                        {
                                            rcx_9 = (*rax_34 - 0x5b);
                                            if ((rcx_9 <= 0x21 && ((1 << rcx_9) & 0x20000002b) != 0))
                                            {
                                                break;
                                            }
                                            rax_34 = &rax_34[1];
                                        } while (rsi_6 != rax_34);
                                        if ((rcx_9 <= 0x21 && ((1 << rcx_9) & 0x20000002b) != 0))
                                        {
                                            r15 = var_88;
                                            r13_1 = 2;
                                            r11 = var_98;
                                            goto label_403d3c;
                                        }
                                    }
                                    mbstate_t* rdi_7 = &var_40;
                                    if (iswprint(var_44) == 0)
                                    {
                                        r13 = 0;
                                    }
                                    rbx_2 = (rbx_2 + rax_12);
                                    if (mbsinit(rdi_7) == 0)
                                    {
                                        continue;
                                    }
                                }
                                rdi_4 = rbx_2;
                                rbp_3 = var_59_1;
                                r10 = var_5a_1;
                                r15 = var_88;
                                r9 = var_98;
                                r11_1 = var_90;
                                rdx = ((r13 ^ 1) & var_c0);
                                break;
                            }
                            if (rdi_4 > 1)
                            {
                                goto label_404421;
                            }
                            goto label_403f24;
                        }
                        switch (rbp_3)
                        {
                            case 0:
                            {
                            label_403e10:
                                if (var_5c != 0)
                                {
                                    r13_1 = r11_1;
                                    r11 = r9;
                                    var_c0 = r13_1 == 2;
                                    goto label_403d3c;
                                }
                                var_b8 = 0;
                            label_403e27:
                                rdx = r11_1 == 2;
                                rax_8 = ((r10 ^ 1) & rdx);
                                int64_t rcx_6;
                                if (rax_8 == 0)
                                {
                                    rcx_6 = rbx_1;
                                    if (r14 <= rbx_1)
                                    {
                                        goto label_403e72;
                                    }
                                    rax_8 = r10;
                                label_403e6a:
                                    r15[rcx_6] = 0x5c;
                                    r10 = rax_8;
                                label_403e72:
                                    rbx_1 = (rcx_6 + 1);
                                    if (r11_1 == 2)
                                    {
                                        rax_8 = r13;
                                        rbp_3 = 0x30;
                                        r13 = 0;
                                        goto label_403c2c;
                                    }
                                    rbp_3 = 0x30;
                                    if ((r12_1 + 1) < r9)
                                    {
                                        char rax_20 = *((arg3 + r12_1) + 1);
                                        var_98 = rax_20;
                                        if ((rax_20 - 0x30) <= 9)
                                        {
                                            if (r14 > rbx_1)
                                            {
                                                r15[rbx_1] = 0x30;
                                            }
                                            if (r14 > (rcx_6 + 2))
                                            {
                                                r15[(rcx_6 + 2)] = 0x30;
                                            }
                                            rbx_1 = (rcx_6 + 3);
                                            rbp_3 = 0x30;
                                        }
                                    }
                                    goto label_403eb3;
                                }
                                if (r14 > rbx_1)
                                {
                                    r15[rbx_1] = 0x27;
                                }
                                if (r14 > (rbx_1 + 1))
                                {
                                    r15[(rbx_1 + 1)] = 0x24;
                                }
                                if (r14 > (rbx_1 + 2))
                                {
                                    r15[(rbx_1 + 2)] = 0x27;
                                }
                                rcx_6 = (rbx_1 + 3);
                                if (r14 > rcx_6)
                                {
                                    goto label_403e6a;
                                }
                                rbx_1 = (rbx_1 + 4);
                                r10 = rax_8;
                                r13 = 0;
                                rbp_3 = 0x30;
                                goto label_403c2c;
                            }
                            case 1:
                            case 2:
                            case 3:
                            case 4:
                            case 5:
                            case 6:
                            case 0xe:
                            case 0xf:
                            case 0x10:
                            case 0x11:
                            case 0x12:
                            case 0x13:
                            case 0x14:
                            case 0x15:
                            case 0x16:
                            case 0x17:
                            case 0x18:
                            case 0x19:
                            case 0x1a:
                            case 0x1b:
                            case 0x1c:
                            case 0x1d:
                            case 0x1e:
                            case 0x1f:
                            case 0x40:
                            {
                                goto label_403ed0;
                            }
                            case 7:
                            {
                                rbp_3 = 0x61;
                                goto label_403de5;
                            }
                            case 8:
                            {
                                rbp_3 = 0x62;
                                goto label_403d1d;
                            }
                            case 9:
                            {
                            label_403da0:
                                rbp_3 = 0x74;
                                goto label_403d1d;
                            }
                            case 0xa:
                            {
                            label_403d18:
                                rbp_3 = 0x6e;
                            label_403d1d:
                                rdx = 0;
                                r13 = 0;
                                if (var_5c != 0)
                                {
                                    goto label_403d30;
                                }
                                goto label_403b78;
                            }
                            case 0xb:
                            {
                            label_403e00:
                                rbp_3 = 0x76;
                                goto label_403de5;
                            }
                            case 0xc:
                            {
                            label_403de0:
                                rbp_3 = 0x66;
                            label_403de5:
                                if (var_5c == 0)
                                {
                                    goto label_403df0;
                                }
                                r13_1 = r11_1;
                                rax_14 = 0;
                                r11 = r9;
                            label_403d38:
                                var_c0 = (var_c0 & rax_14);
                                goto label_403d3c;
                            }
                            case 0xd:
                            {
                                var_b8 = 0;
                            label_403dc9:
                                rbp_3 = 0xd;
                                rcx_2 = 0x72;
                                rdx = r11_1 == 2;
                                goto label_403d00;
                            }
                            case 0x20:
                            {
                                rax_27 = 0;
                            label_40412a:
                                rbp_3 = 0x20;
                                goto label_4040f2;
                            }
                            case 0x21:
                            case 0x22:
                            case 0x24:
                            case 0x26:
                            case 0x28:
                            case 0x29:
                            case 0x2a:
                            case 0x3b:
                            case 0x3c:
                            case 0x3d:
                            case 0x3e:
                            case 0x5b:
                            case 0x5e:
                            case 0x60:
                            case 0x7c:
                            {
                                var_b8 = 0;
                            label_403db5:
                                rdx = 0;
                            label_403d0b:
                                r13 = 0;
                                goto label_403b25;
                            }
                            case 0x23:
                            case 0x7e:
                            {
                                rax_28 = 0;
                            label_40410a:
                                if (r12_1 == 0)
                                {
                                    r13 = var_b8;
                                    rdx = 0;
                                    var_b8 = rax_28;
                                    goto label_403b25;
                                }
                                var_b8 = rax_28;
                                r13 = 0;
                            label_403b2f:
                                rax_8 = 0;
                                goto label_403b31;
                            }
                            case 0x25:
                            case 0x2b:
                            case 0x2c:
                            case 0x2d:
                            case 0x2e:
                            case 0x2f:
                            case 0x30:
                            case 0x31:
                            case 0x32:
                            case 0x33:
                            case 0x34:
                            case 0x35:
                            case 0x36:
                            case 0x37:
                            case 0x38:
                            case 0x39:
                            case 0x3a:
                            case 0x41:
                            case 0x42:
                            case 0x43:
                            case 0x44:
                            case 0x45:
                            case 0x46:
                            case 0x47:
                            case 0x48:
                            case 0x49:
                            case 0x4a:
                            case 0x4b:
                            case 0x4c:
                            case 0x4d:
                            case 0x4e:
                            case 0x4f:
                            case 0x50:
                            case 0x51:
                            case 0x52:
                            case 0x53:
                            case 0x54:
                            case 0x55:
                            case 0x56:
                            case 0x57:
                            case 0x58:
                            case 0x59:
                            case 0x5a:
                            case 0x5d:
                            case 0x5f:
                            case 0x61:
                            case 0x62:
                            case 0x63:
                            case 0x64:
                            case 0x65:
                            case 0x66:
                            case 0x67:
                            case 0x68:
                            case 0x69:
                            case 0x6a:
                            case 0x6b:
                            case 0x6c:
                            case 0x6d:
                            case 0x6e:
                            case 0x6f:
                            case 0x70:
                            case 0x71:
                            case 0x72:
                            case 0x73:
                            case 0x74:
                            case 0x75:
                            case 0x76:
                            case 0x77:
                            case 0x78:
                            case 0x79:
                            case 0x7a:
                            {
                                rax_27 = 0;
                            label_4040f2:
                                r13 = var_b8;
                                var_b8 = rax_27;
                                rax_8 = 0;
                                goto label_403b31;
                            }
                            case 0x27:
                            {
                                var_b8 = 0;
                            label_403cad:
                                if (r11_1 != 2)
                                {
                                    var_5b_1 = r13;
                                    rdx = 0;
                                    rbp_3 = 0x27;
                                    goto label_403b25;
                                }
                                if (var_5c != 0)
                                {
                                    goto label_404a6d;
                                }
                                if (r14 != 0)
                                {
                                    rdx = 0;
                                }
                                if ((r14 == 0 || (r14 != 0 && var_70 != 0)))
                                {
                                    if (r14 > rbx_1)
                                    {
                                        r15[rbx_1] = 0x27;
                                    }
                                    if (r14 > (rbx_1 + 1))
                                    {
                                        r15[(rbx_1 + 1)] = 0x5c;
                                    }
                                    if (r14 <= (rbx_1 + 2))
                                    {
                                        rdx = r14;
                                        r14 = var_70;
                                    }
                                    else
                                    {
                                        rdx = r14;
                                        r15[(rbx_1 + 2)] = 0x27;
                                        r14 = var_70;
                                    }
                                }
                                var_5b_1 = r13;
                                rbx_1 = (rbx_1 + 3);
                                rax_8 = 0;
                                r10 = 0;
                                var_70 = r14;
                                rbp_3 = 0x27;
                                r14 = rdx;
                                goto label_403c2c;
                            }
                            case 0x3f:
                            {
                                var_b8 = 0;
                            label_403c7d:
                                if (r11_1 == 2)
                                {
                                    if (var_5c != 0)
                                    {
                                    label_404a6d:
                                        r13_1 = r11_1;
                                        r11 = r9;
                                        goto label_403d3c;
                                    }
                                    r13 = 0;
                                    rax_8 = 0;
                                    rbp_3 = 0x3f;
                                label_403c2c:
                                    rdx = r11_1 == 2;
                                    if (var_b8 != 0)
                                    {
                                        goto label_403b67;
                                    }
                                    r12_1 = (r12_1 + 1);
                                    rax_12 = ((rax_8 ^ 1) & r10);
                                    goto label_403c44;
                                }
                                if (r11_1 != 5)
                                {
                                    rbp_3 = 0x3f;
                                    rdx = r11_1 == 2;
                                    r13 = 0;
                                    goto label_403b25;
                                }
                                if ((arg6 & 4) == 0)
                                {
                                label_404210:
                                    rdx = 0;
                                    r13 = 0;
                                    rbp_3 = 0x3f;
                                    goto label_403b25;
                                }
                                int64_t rax_30 = (r12_1 + 2);
                                if (rax_30 >= r9)
                                {
                                    goto label_404210;
                                }
                                if (*((arg3 + r12_1) + 1) != 0x3f)
                                {
                                    goto label_404210;
                                }
                                rbp_3 = *(arg3 + rax_30);
                                if (rbp_3 > 0x3e)
                                {
                                    goto label_404210;
                                }
                                if ((!(TEST_BITQ(0x7000a38200000000, rbp_3))))
                                {
                                    goto label_404210;
                                }
                                if (var_5c != 0)
                                {
                                    goto label_404a20;
                                }
                                if (r14 > rbx_1)
                                {
                                    r15[rbx_1] = 0x3f;
                                }
                                if (r14 > (rbx_1 + 1))
                                {
                                    r15[(rbx_1 + 1)] = 0x22;
                                }
                                if (r14 > (rbx_1 + 2))
                                {
                                    r15[(rbx_1 + 2)] = 0x22;
                                }
                                if (r14 > (rbx_1 + 3))
                                {
                                    r15[(rbx_1 + 3)] = 0x3f;
                                }
                                rbx_1 = (rbx_1 + 4);
                                rdx = 0;
                                r13 = 0;
                                r12_1 = rax_30;
                            label_403eb3:
                                rax_8 = r13;
                                r13 = 0;
                                if (((var_c0 ^ 1) | rdx) != 0)
                                {
                                    goto label_403c2c;
                                }
                            label_403b31:
                                if (arg7 == 0)
                                {
                                    goto label_403c2c;
                                }
                                if (((*(arg7 + ((rbp_3 >> 5) << 2)) >> rbp_3) & 1) == 0)
                                {
                                    goto label_403c2c;
                                }
                                rdx = r11_1 == 2;
                                goto label_403b67;
                            }
                            case 0x5c:
                            {
                                var_b8 = 0;
                            label_403cda:
                                rdx = (var_c0 & var_5c);
                                rbp_3 = 0x5c;
                                rcx_2 = 0x5c;
                                rax_1 = var_a0_1 != 0;
                                rdx = (rdx & rax_1);
                                if (rdx == 0)
                                {
                                    goto label_403d00;
                                }
                                r12_1 = (r12_1 + 1);
                                rax_12 = r10;
                                r13 = 0;
                            label_403c44:
                                if (rax_12 != 0)
                                {
                                    if (r14 > rbx_1)
                                    {
                                        r15[rbx_1] = 0x27;
                                    }
                                    if (r14 > (rbx_1 + 1))
                                    {
                                        r15[(rbx_1 + 1)] = 0x27;
                                    }
                                    rbx_1 = (rbx_1 + 2);
                                    r10 = 0;
                                    goto label_403bbd;
                                }
                            label_403bbd:
                                if (rbx_1 >= r14)
                                {
                                    goto label_403bc6;
                                }
                                r15[rbx_1] = rbp_3;
                            label_403bc6:
                                char rdi_2 = var_a9;
                                rbx_1 = (rbx_1 + 1);
                                if (r13 == 0)
                                {
                                    rdi_2 = 0;
                                }
                                var_a9 = rdi_2;
                                continue;
                            }
                            case 0x7b:
                            case 0x7d:
                            {
                                var_b8 = 0;
                            label_403ad9:
                                rax_1 = r9 != 1;
                                if (r9 == -1)
                                {
                                    rax_1 = *(arg3 + 1) != 0;
                                }
                                rdx = r11_1 == 2;
                                if (rax_1 != 0)
                                {
                                    goto label_403d0b;
                                }
                                goto label_403afd;
                            }
                        }
                    }
                }
            }
        label_4041b6:
            *r15 = 0x27;
            r13 = 2;
            rbx_1 = 1;
            var_78_1 = "'";
            var_a0_1 = 1;
            var_5c = false;
            var_5b_1 = rax_1;
        }
    }
    if ((r13 == 4 && rbp_1 == 0))
    {
        var_c0 = 1;
    }
    switch (r13)
    {
        case 0:
        {
            var_a9 = 1;
            r10 = 0;
            rbx_1 = 0;
            var_5b_1 = 0;
            var_70 = 0;
            var_5c = false;
            var_c0 = 0;
            var_a0_1 = 0;
            var_78_1 = nullptr;
            goto label_4039c8;
        }
        case 2:
        {
            if (rbp_1 != 0)
            {
                var_a9 = 1;
                r10 = 0;
                rbx_1 = 0;
                var_5b_1 = 0;
                var_70 = 0;
                var_5c = true;
                var_c0 = 0;
                var_a0_1 = 1;
                var_78_1 = "'";
                goto label_4039c8;
            }
            var_c0 = 0;
            break;
        }
        case 3:
        {
            var_a9 = 1;
            r10 = 0;
            rbx_1 = 0;
            r13 = 2;
            var_5b_1 = 0;
            var_70 = 0;
            var_5c = true;
            var_c0 = 1;
            var_a0_1 = 1;
            var_78_1 = "'";
            goto label_4039c8;
        }
        case 5:
        {
            if (rbp_1 != 0)
            {
                var_a9 = 1;
                r10 = 0;
                rbx_1 = 0;
                var_5b_1 = 0;
                var_70 = 0;
                var_5c = true;
                var_c0 = 1;
                var_a0_1 = 1;
                var_78_1 = &data_4082f4;
            }
            else
            {
                if (r14 == 0)
                {
                    var_70 = 0;
                    r10 = 0;
                    var_a9 = 1;
                    var_5b_1 = 0;
                }
                else
                {
                    *r15 = 0x22;
                    r10 = 0;
                    var_a9 = 1;
                    var_5b_1 = 0;
                    var_70 = 0;
                }
                var_5c = false;
                rbx_1 = 1;
                var_c0 = 1;
                var_a0_1 = 1;
                var_78_1 = &data_4082f4;
            }
            goto label_4039c8;
        }
        case 6:
        {
            var_a9 = 1;
            r10 = 0;
            rbx_1 = 0;
            r13 = 5;
            var_5b_1 = 0;
            var_70 = 0;
            var_5c = true;
            var_c0 = 1;
            var_a0_1 = 1;
            var_78_1 = &data_4082f4;
            goto label_4039c8;
        }
        case 7:
        {
            var_a9 = 1;
            r10 = 0;
            rbx_1 = 0;
            var_5b_1 = 0;
            var_70 = 0;
            var_5c = false;
            var_c0 = 1;
            var_a0_1 = 0;
            var_78_1 = nullptr;
            goto label_4039c8;
        }
        case 8:
        case 9:
        case 0xa:
        {
            if ((!cond:1))
            {
                arg8 = sub_403830(&data_408305, r13);
                arg9 = sub_403830("'", r13);
            }
            rbx_1 = 0;
            if (rbp_1 == 0)
            {
                char rax_49 = *arg8;
                if (rax_49 != 0)
                {
                    do
                    {
                        if (r14 > rbx_1)
                        {
                            r15[rbx_1] = rax_49;
                        }
                        rbx_1 = (rbx_1 + 1);
                        rax_49 = arg8[rbx_1];
                    } while (rax_49 != 0);
                }
            }
            uint64_t rax_46;
            rax_46 = strlen(arg9);
            var_a9 = 1;
            r10 = 0;
            var_a0_1 = rax_46;
            var_5b_1 = 0;
            var_78_1 = arg9;
            var_70 = 0;
            var_c0 = 1;
            goto label_4039c8;
        }
    }
    if (((r13 == 4 && rbp_1 == 0) || r13 == 2))
    {
        if (r14 != 0)
        {
            r10 = 0;
            rax_1 = 0;
            var_a9 = 1;
            var_70 = 0;
            goto label_4041b6;
        }
        var_a9 = 1;
        r10 = 0;
        rbx_1 = 1;
        r13 = 2;
        var_70 = 0;
        var_5b_1 = 0;
        var_5c = false;
        var_a0_1 = 1;
        var_78_1 = "'";
        goto label_4039c8;
    }
}

char* sub_404b60(int32_t arg1, uint64_t arg2, uint64_t arg3, int32_t* arg4)
{
    int64_t r15 = arg1;
    int32_t* rax = __errno_location();
    int64_t rbx = data_60b298;
    int32_t rax_1 = *rax;
    if (r15 < 0)
    {
        abort();
        /* no return */
    }
    if (data_60b2b0 <= r15)
    {
        if (r15 == 0x7fffffff)
        {
            sub_405bc0();
            /* no return */
        }
        int32_t r12_1 = (r15 + 1);
        int64_t rsi_1 = (r12_1 << 4);
        if (rbx == 0x60b2a0)
        {
            int64_t rax_9 = sub_4059d0(0, rsi_1);
            int128_t zmm0 = data_60b2a0;
            rbx = rax_9;
            data_60b298 = rax_9;
            *rax_9 = zmm0;
        }
        else
        {
            int64_t rax_2 = sub_4059d0(rbx, rsi_1);
            data_60b298 = rax_2;
            rbx = rax_2;
        }
        int64_t rdi_1 = data_60b2b0;
        memset(((rdi_1 << 4) + rbx), 0, ((r12_1 - rdi_1) << 4));
        data_60b2b0 = r12_1;
    }
    int64_t rbx_1 = (rbx + (r15 << 4));
    void* r15_2 = &arg4[2];
    int32_t rax_4 = (arg4[1] | 1);
    uint64_t r11 = *rbx_1;
    char* r12_2 = *(rbx_1 + 8);
    int64_t rax_5 = sub_403930(r12_2, r11, arg2, arg3, *arg4, rax_4, r15_2, *(arg4 + 0x28), *(arg4 + 0x30));
    if (r11 <= rax_5)
    {
        uint64_t rsi_3 = (rax_5 + 1);
        *rbx_1 = rsi_3;
        if (r12_2 != 0x60b360)
        {
            free(r12_2);
        }
        char* rax_6 = sub_405970(rsi_3);
        uint32_t r8_1 = *arg4;
        *(rbx_1 + 8) = rax_6;
        r12_2 = rax_6;
        sub_403930(rax_6, rsi_3, arg2, arg3, r8_1, rax_4, r15_2, *(arg4 + 0x28), *(arg4 + 0x30));
    }
    *rax = rax_1;
    return r12_2;
}

int64_t sub_404d00(int64_t arg1)
{
    int64_t rbx = arg1;
    int32_t* rax = __errno_location();
    int32_t r12 = *rax;
    if (rbx == 0)
    {
        rbx = 0x60b460;
    }
    int64_t rax_1 = sub_405b70(rbx, 0x38);
    *rax = r12;
    return rax_1;
}

uint64_t sub_404d40(int32_t* arg1)
{
    if (arg1 == 0)
    {
        arg1 = &data_60b460;
    }
    return *arg1;
}

void sub_404d50(int32_t* arg1, int32_t arg2)
{
    if (arg1 == 0)
    {
        arg1 = &data_60b460;
    }
    *arg1 = arg2;
}

uint64_t sub_404d60(int64_t arg1, uint8_t arg2, int32_t arg3)
{
    if (arg1 == 0)
    {
        arg1 = 0x60b460;
    }
    char rcx_1 = (arg2 & 0x1f);
    int32_t rsi = *((arg1 + ((arg2 >> 5) << 2)) + 8);
    uint32_t rax_3 = (rsi >> rcx_1);
    *((arg1 + ((arg2 >> 5) << 2)) + 8) = ((((arg3 ^ rax_3) & 1) << rcx_1) ^ rsi);
    return (rax_3 & 1);
}

uint64_t sub_404da0(int128_t* arg1, int32_t arg2)
{
    if (arg1 == 0)
    {
        arg1 = &data_60b460;
    }
    uint64_t rax = *(arg1 + 4);
    *(arg1 + 4) = arg2;
    return rax;
}

void sub_404dc0(int32_t* arg1, int64_t arg2, int64_t arg3)
{
    if (arg1 == 0)
    {
        arg1 = &data_60b460;
    }
    *arg1 = 0xa;
    if ((arg2 != 0 && arg3 != 0))
    {
        *(arg1 + 0x28) = arg2;
        *(arg1 + 0x30) = arg3;
        return;
    }
    int64_t var_8 = 0x60b460;
    abort();
    /* no return */
}

int64_t sub_404df0(char* arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, 
    int32_t* arg5)
{
    int32_t* rbx = arg5;
    if (arg5 == 0)
    {
        rbx = &data_60b460;
    }
    int32_t* rax = __errno_location();
    int32_t r12 = *rax;
    int64_t rax_2 = sub_403930(arg1, arg2, arg3, arg4, *rbx, rbx[1], &rbx[2], *(rbx + 0x28), *(rbx + 0x30));
    *rax = r12;
    return rax_2;
}

char* sub_404e70(uint64_t arg1, uint64_t arg2, int64_t* arg3, int32_t* arg4)
{
    int32_t* rbx = arg4;
    if (arg4 == 0)
    {
        rbx = &data_60b460;
    }
    int32_t* rax = __errno_location();
    int32_t rax_1 = *rax;
    int32_t rbp;
    rbp = arg3 == 0;
    int32_t rbp_1 = (rbp | rbx[1]);
    int64_t rax_2 = sub_403930(nullptr, 0, arg1, arg2, *rbx, rbp_1, &rbx[2], *(rbx + 0x28), *(rbx + 0x30));
    char* rax_3 = sub_405970((rax_2 + 1));
    sub_403930(rax_3, (rax_2 + 1), arg1, arg2, *rbx, rbp_1, &rbx[2], *(rbx + 0x28), *(rbx + 0x30));
    *rax = rax_1;
    if (arg3 != 0)
    {
        *arg3 = rax_2;
    }
    return rax_3;
}

int64_t sub_404f60(uint64_t arg1, uint64_t arg2, int32_t* arg3)
{
    /* tailcall */
    return sub_404e70(arg1, arg2, nullptr, arg3);
}

uint64_t sub_404f70()
{
    uint64_t rax_2 = data_60b2b0;
    int64_t r12 = data_60b298;
    if (rax_2 > 1)
    {
        int64_t* rbx_1 = (r12 + 0x18);
        int64_t rbp_1 = ((r12 + ((rax_2 - 2) << 4)) + 0x28);
        do
        {
            int64_t rdi_1 = *rbx_1;
            rbx_1 = &rbx_1[2];
            rax_2 = free(rdi_1);
        } while (rbx_1 != rbp_1);
    }
    int64_t rdi_2 = *(r12 + 8);
    if (rdi_2 != 0x60b360)
    {
        rax_2 = free(rdi_2);
        *data_60b2a0 = 0x100;
        *(data_60b2a0 + 8) = 0x60b360;
    }
    if (r12 != 0x60b2a0)
    {
        rax_2 = free(r12);
        data_60b298 = 0x60b2a0;
    }
    data_60b2b0 = 1;
    return rax_2;
}

int64_t sub_405010(int32_t arg1, uint64_t arg2)
{
    /* tailcall */
    return sub_404b60(arg1, arg2, -1, &data_60b460);
}

int64_t sub_405030(int32_t arg1, uint64_t arg2, uint64_t arg3)
{
    /* tailcall */
    return sub_404b60(arg1, arg2, arg3, &data_60b460);
}

int64_t sub_405040(uint64_t arg1)
{
    /* tailcall */
    return sub_404b60(0, arg1, -1, &data_60b460);
}

int64_t sub_405060(uint64_t arg1, uint64_t arg2)
{
    /* tailcall */
    return sub_404b60(0, arg1, arg2, &data_60b460);
}

char* sub_405080(int32_t arg1, int32_t arg2, uint64_t arg3)
{
    if (arg2 == 0xa)
    {
        abort();
        /* no return */
    }
    int32_t var_48 = arg2;
    int32_t var_44 = 0;
    int64_t var_40 = 0;
    int64_t var_38 = 0;
    int64_t var_30 = 0;
    int64_t var_28 = 0;
    int64_t var_20 = 0;
    int64_t var_18 = 0;
    return sub_404b60(arg1, arg3, -1, &var_48);
}

char* sub_4050f0(int32_t arg1, int32_t arg2, uint64_t arg3, uint64_t arg4)
{
    if (arg2 == 0xa)
    {
        abort();
        /* no return */
    }
    int32_t var_48 = arg2;
    int32_t var_44 = 0;
    int64_t var_40 = 0;
    int64_t var_38 = 0;
    int64_t var_30 = 0;
    int64_t var_28 = 0;
    int64_t var_20 = 0;
    int64_t var_18 = 0;
    return sub_404b60(arg1, arg3, arg4, &var_48);
}

int64_t sub_405160(int32_t arg1, uint64_t arg2)
{
    /* tailcall */
    return sub_405080(0, arg1, arg2);
}

int64_t sub_405170(int32_t arg1, uint64_t arg2, uint64_t arg3)
{
    /* tailcall */
    return sub_4050f0(0, arg1, arg2, arg3);
}

char* sub_405180(uint64_t arg1, uint64_t arg2, uint8_t arg3)
{
    char rcx_1 = (arg3 & 0x1f);
    int64_t var_18 = data_60b490;
    int128_t var_48 = data_60b460;
    int128_t var_38 = data_60b470;
    int128_t var_28 = data_60b480;
    int32_t rdx = *((&var_48 + ((arg3 >> 5) << 2)) + 8);
    *((&var_48 + ((arg3 >> 5) << 2)) + 8) = ((((!(rdx >> rcx_1)) & 1) << rcx_1) ^ rdx);
    return sub_404b60(0, arg1, arg2, &var_48);
}

int64_t sub_4051f0(uint64_t arg1, uint8_t arg2)
{
    /* tailcall */
    return sub_405180(arg1, -1, arg2);
}

int64_t sub_405200(uint64_t arg1)
{
    /* tailcall */
    return sub_405180(arg1, -1, 0x3a);
}

int64_t sub_405220(uint64_t arg1, uint64_t arg2)
{
    /* tailcall */
    return sub_405180(arg1, arg2, 0x3a);
}

char* sub_405230(int32_t arg1, int32_t arg2, uint64_t arg3)
{
    if (arg2 == 0xa)
    {
        abort();
        /* no return */
    }
    int32_t var_48 = arg2;
    int64_t var_40 = 0x400000000000000;
    int32_t var_44 = 0;
    int64_t var_38 = 0;
    int64_t var_30 = 0;
    int64_t var_28 = 0;
    int64_t var_20 = 0;
    int64_t var_18 = 0;
    return sub_404b60(arg1, arg3, -1, &var_48);
}

char* sub_4052a0(int32_t arg1, int64_t arg2, int64_t arg3, uint64_t arg4, 
    uint64_t arg5)
{
    int128_t var_48 = data_60b460;
    int128_t var_38 = data_60b470;
    int128_t var_28 = data_60b480;
    int64_t var_18 = data_60b490;
    var_48 = 0xa;
    if ((arg2 != 0 && arg3 != 0))
    {
        *var_28[8] = arg2;
        int64_t var_18_1 = arg3;
        return sub_404b60(arg1, arg4, arg5, &var_48);
    }
    abort();
    /* no return */
}

int64_t sub_4053b0(uint64_t arg1)
{
    /* tailcall */
    return sub_404b60(0, arg1, -1, &data_60b260);
}

int64_t sub_4053d0(FILE* arg1, int64_t arg2, int64_t arg3, int64_t arg4, 
    int64_t* arg5, int64_t arg6)
{
    if (arg2 == 0)
    {
        __fprintf_chk();
    }
    else
    {
        __fprintf_chk();
    }
    __fprintf_chk(arg1, 1, "Copyright %s %d Free Software Fo…", dcgettext(nullptr, &data_40904b, 5), 0x7e1);
    fputs_unlocked(dcgettext(nullptr, "\nLicense GPLv3+: GNU GPL versio…", 5), arg1);
    int64_t var_58;
    int64_t var_50_1;
    int64_t var_48;
    int64_t var_40;
    char* rax_6;
    int64_t rbx_1;
    char* rsi_1;
    int64_t r12_1;
    int64_t r13;
    int64_t r14;
    int64_t r15_1;
    switch (arg6)
    {
        case 0:
        {
            abort();
            /* no return */
        }
        case 8:
        {
            r13 = arg5[6];
            r12_1 = arg5[5];
            r15_1 = arg5[4];
            var_48 = arg5[7];
            r14 = arg5[3];
            rbx_1 = *arg5;
            var_58 = arg5[2];
            var_50_1 = arg5[1];
            rax_6 = dcgettext(nullptr, "Written by %s, %s, %s,\n%s, %s, …", 5);
            break;
        }
        case 9:
        {
            rsi_1 = "Written by %s, %s, %s,\n%s, %s, …";
            r13 = arg5[6];
            r12_1 = arg5[5];
            var_40 = arg5[8];
            r15_1 = arg5[4];
            r14 = arg5[3];
            var_48 = arg5[7];
            var_58 = arg5[2];
            rbx_1 = *arg5;
            var_50_1 = arg5[1];
            break;
        }
        default:
            rsi_1 = "Written by %s, %s, %s,\n%s, %s, …";
            r13 = arg5[6];
            r12_1 = arg5[5];
            var_40 = arg5[8];
            r15_1 = arg5[4];
            r14 = arg5[3];
            var_48 = arg5[7];
            var_58 = arg5[2];
            rbx_1 = *arg5;
            var_50_1 = arg5[1];
    }
    int64_t var_60;
    if ((arg6 > 9 || arg6 == 9))
    {
        rax_6 = dcgettext(nullptr, rsi_1, 5);
        var_60 = var_40;
    }
    int64_t var_78_1;
    int64_t var_70_1;
    int64_t var_68_2;
    int64_t var_58_1;
    int64_t var_50;
    char* rax_19;
    char* rax_22;
    int64_t rbx_5;
    int64_t rbx_6;
    int64_t r9_9;
    int64_t r12_4;
    int64_t r13_2;
    int64_t r14_1;
    switch (arg6)
    {
        case 1:
        {
            /* tailcall */
            return __fprintf_chk(arg1, 1, dcgettext(nullptr, "Written by %s.\n", 5), *arg5);
            break;
        }
        case 2:
        {
            /* tailcall */
            return __fprintf_chk(arg1, 1, dcgettext(nullptr, "Written by %s and %s.\n", 5), *arg5, arg5[1]);
            break;
        }
        case 3:
        {
            /* tailcall */
            return __fprintf_chk(arg1, 1, dcgettext(nullptr, "Written by %s, %s, and %s.\n", 5), *arg5, arg5[1], arg5[2]);
            break;
        }
        case 4:
        {
            r14_1 = arg5[3];
            r13_2 = arg5[2];
            r12_4 = arg5[1];
            rbx_5 = *arg5;
            rax_19 = dcgettext(nullptr, "Written by %s, %s, %s,\nand %s.\n", 5);
            break;
        }
        case 5:
        {
            int64_t r15_2 = arg5[4];
            r14_1 = arg5[3];
            r13_2 = arg5[2];
            r12_4 = arg5[1];
            rbx_5 = *arg5;
            rax_19 = dcgettext(nullptr, "Written by %s, %s, %s,\n%s, and …", 5);
            var_60 = r15_2;
            break;
        }
        case 6:
        {
            int64_t r12_5 = arg5[5];
            int64_t r15_3 = arg5[4];
            int64_t r14_2 = arg5[3];
            var_58_1 = arg5[1];
            int64_t r13_3 = arg5[2];
            rbx_6 = *arg5;
            rax_22 = dcgettext(nullptr, "Written by %s, %s, %s,\n%s, %s, …", 5);
            r9_9 = r13_3;
            var_68_2 = r12_5;
            var_70_1 = r15_3;
            var_78_1 = r14_2;
            break;
        }
        case 7:
        {
            int64_t r13_4 = arg5[6];
            int64_t r12_6 = arg5[5];
            int64_t r15_4 = arg5[4];
            int64_t r14_3 = arg5[3];
            var_50 = arg5[2];
            rbx_6 = *arg5;
            var_58_1 = arg5[1];
            rax_22 = dcgettext(nullptr, "Written by %s, %s, %s,\n%s, %s, …", 5);
            var_60 = r13_4;
            var_68_2 = r12_6;
            var_70_1 = r15_4;
            var_78_1 = r14_3;
            r9_9 = var_50;
            break;
        }
        default:
            return __fprintf_chk(arg1, 1, rax_6, rbx_1, var_50_1, var_58, r14, r15_1, r12_1, r13, var_48, var_60, var_58, var_50_1, var_48, var_40);
    }
    if ((arg6 == 4 || arg6 == 5))
    {
        __fprintf_chk(arg1, 1, rax_19, rbx_5, r12_4, r13_2, r14_1, var_60);
        return r14_1;
    }
    else if ((arg6 == 6 || arg6 == 7))
    {
        return __fprintf_chk(arg1, 1, rax_22, rbx_6, var_58_1, r9_9, var_78_1, var_70_1, var_68_2, var_60, var_58_1, var_50);
    }
}

int64_t sub_4057b0(FILE* arg1, int64_t arg2, int64_t arg3, int64_t arg4, 
    int64_t* arg5)
{
    int64_t r9 = 0;
    if (*arg5 != 0)
    {
        do
        {
            r9 = (r9 + 1);
        } while (arg5[r9] != 0);
    }
    /* tailcall */
    return sub_4053d0(arg1, arg2, arg3, arg4, arg5, r9);
}

int64_t sub_4057d0(FILE* arg1, int64_t arg2, int64_t arg3, int64_t arg4, 
    int32_t* arg5)
{
    int64_t r9 = 0;
    void var_58;
    do
    {
        int32_t rax_3 = *arg5;
        if (rax_3 <= 0x2f)
        {
            int64_t* r10_2 = (rax_3 + *(arg5 + 0x10));
            *arg5 = (rax_3 + 8);
            int64_t rax_2 = *r10_2;
            *(&var_58 + (r9 << 3)) = rax_2;
            if (rax_2 == 0)
            {
                break;
            }
        }
        else
        {
            int64_t* r10_3 = *(arg5 + 8);
            *(arg5 + 8) = &r10_3[1];
            int64_t rax_5 = *r10_3;
            *(&var_58 + (r9 << 3)) = rax_5;
            if (rax_5 == 0)
            {
                break;
            }
        }
        r9 = (r9 + 1);
    } while (r9 != 0xa);
    return sub_4053d0(arg1, arg2, arg3, arg4, &var_58, r9);
}

int64_t sub_405830(FILE* arg1, int64_t arg2, int64_t arg3, int64_t arg4)
{
    char r11 = 0;
    int64_t r8;
    int64_t var_18 = r8;
    void* r10 = &arg_8;
    int32_t r8_1 = 0x20;
    void* var_98 = &arg_8;
    int64_t r9;
    int64_t var_10 = r9;
    int64_t r9_1 = 0;
    int32_t var_a0 = 0x20;
    void var_38;
    void* var_90 = &var_38;
    void var_88;
    do
    {
        if (r8_1 <= 0x2f)
        {
            uint64_t rax_1 = r8_1;
            r8_1 = (r8_1 + 8);
            r11 = 1;
            int64_t rax_3 = *(rax_1 + &var_38);
            *(&var_88 + (r9_1 << 3)) = rax_3;
            if (rax_3 == 0)
            {
                break;
            }
        }
        else
        {
            void* rax_4 = r10;
            r10 = (r10 + 8);
            int64_t rax_5 = *rax_4;
            *(&var_88 + (r9_1 << 3)) = rax_5;
            if (rax_5 == 0)
            {
                break;
            }
        }
        r9_1 = (r9_1 + 1);
    } while (r9_1 != 0xa);
    if (r11 != 0)
    {
        int32_t var_a0_1 = r8_1;
    }
    return sub_4053d0(arg1, arg2, arg3, arg4, &var_88, r9_1);
}

int64_t sub_4058f0()
{
    __printf_chk(1, dcgettext(nullptr, "\nReport bugs to: %s\n", 5), "bug-coreutils@gnu.org");
    __printf_chk(1, dcgettext(nullptr, "%s home page: <%s>\n", 5), "GNU coreutils", "https://www.gnu.org/software/cor…");
    /* tailcall */
    return fputs_unlocked(dcgettext(nullptr, "General help using GNU software:…", 5), stdout);
}

int64_t sub_405970(uint64_t arg1)
{
    int64_t rax = malloc(arg1);
    if ((rax == 0 && arg1 != 0))
    {
        sub_405bc0();
        /* no return */
    }
    return rax;
}

int64_t sub_4059d0(int64_t arg1, uint64_t arg2)
{
    if ((arg2 == 0 && arg1 != 0))
    {
        free(arg1);
        return 0;
    }
    int64_t rax = realloc(arg1, arg2);
    if ((rax == 0 && arg2 != 0))
    {
        sub_405bc0();
        /* no return */
    }
    return rax;
}

int64_t sub_405b1a() __noreturn
{
    sub_405bc0();
    /* no return */
}

int64_t sub_405b20(uint64_t arg1)
{
    /* tailcall */
    return memset(sub_405970(arg1), 0, arg1);
}

int64_t sub_405b40(uint64_t arg1, uint64_t arg2)
{
    int64_t rax;
    bool rdx;
    rdx = HIGHD((arg1 * arg2));
    rax = LOWD((arg1 * arg2));
    if ((rax >= 0 && ((arg1 * arg2) >> 0x40) != 0 == 0))
    {
        int64_t rax_1 = calloc(arg1, arg2);
        if (rax_1 != 0)
        {
            return rax_1;
        }
    }
    sub_405bc0();
    /* no return */
}

int64_t sub_405b70(int64_t arg1, uint64_t arg2)
{
    /* tailcall */
    return memcpy(sub_405970(arg2), arg1, arg2);
}

int64_t sub_405ba0(char* arg1)
{
    /* tailcall */
    return sub_405b70(arg1, (strlen(arg1) + 1));
}

int64_t sub_405bc0() __noreturn
{
    error(data_60b240, 0, "%s", dcgettext(nullptr, "memory exhausted", 5));
    abort();
    /* no return */
}

char* sub_405c00(char* arg1)
{
    char* rax = strndup(arg1);
    if (rax == 0)
    {
        sub_405bc0();
        /* no return */
    }
    return rax;
}

int64_t sub_405c20(FILE* arg1)
{
    FILE* rdi = arg1;
    if (fileno(arg1) >= 0)
    {
        int32_t rax_1 = __freading(rdi);
        off64_t rax_6;
        if (rax_1 != 0)
        {
            rax_6 = lseek(fileno(arg1), 0, 1);
        }
        if (((rax_1 == 0 || (rax_1 != 0 && rax_6 != -1)) && sub_405ca0(arg1) != 0))
        {
            int32_t* rax_3 = __errno_location();
            int32_t r12 = *rax_3;
            int64_t rax_4 = fclose(arg1);
            if (r12 != 0)
            {
                *rax_3 = r12;
                rax_4 = 0xffffffff;
            }
            return rax_4;
        }
        rdi = arg1;
    }
    /* tailcall */
    return fclose(rdi);
}

int64_t sub_405ca0(FILE* arg1)
{
    int32_t* rbx = arg1;
    if ((arg1 != 0 && (__freading(arg1) != 0 && (*rbx & 0x100) != 0)))
    {
        sub_405d10(rbx, 0, 1);
        /* tailcall */
        return fflush(rbx);
    }
    /* tailcall */
    return fflush(rbx);
}

int64_t sub_405ce0(void* arg1, int64_t* arg2)
{
    if (*(arg1 + 0x28) <= *(arg1 + 0x20))
    {
        int64_t rax = *(arg1 + 8);
        int64_t rdx_1 = *(arg1 + 0x10);
        int64_t rdx_2 = (rdx_1 - rax);
        if (rdx_1 != rax)
        {
            *arg2 = rdx_2;
            return rax;
        }
    }
    return 0;
}

off64_t sub_405d10(FILE* arg1, off_t arg2, int32_t arg3)
{
    if ((arg1->_IO_read_end == arg1->_IO_read_ptr && (arg1->_IO_write_ptr == arg1->_IO_write_base && arg1->_IO_save_base == 0)))
    {
        off64_t rax_3 = lseek(fileno(arg1), arg2, arg3);
        if (rax_3 != -1)
        {
            arg1->_flags = (arg1->_flags & 0xffffffef);
            arg1->_offset = rax_3;
            rax_3 = 0;
        }
        return rax_3;
    }
    /* tailcall */
    return fseeko(arg1, arg2, arg3);
}

uint64_t sub_405d70(int64_t arg1, int32_t* arg2)
{
    int64_t r11 = (arg1 + 8);
    int32_t r12 = arg2[0xb];
    int32_t rbp = *arg2;
    int64_t rbx = arg2[0xc];
    int32_t r9 = r12;
    int32_t r10 = rbp;
    int32_t r8 = rbx;
    while (r10 > r8)
    {
        int32_t r14_2;
        while (r9 < r8)
        {
            r14_2 = (r10 - r8);
            int32_t rdx_2 = (r8 - r9);
            if (r14_2 <= rdx_2)
            {
                break;
            }
            int64_t rcx_1 = r9;
            r10 = (r10 - rdx_2);
            int64_t* rax_1 = (arg1 + (rcx_1 << 3));
            int64_t r13_1 = (r11 + (((rdx_2 - 1) + rcx_1) << 3));
            int64_t rdx_6 = (r10 - rcx_1);
            do
            {
                int64_t rcx_2 = *rax_1;
                *rax_1 = rax_1[rdx_6];
                rax_1[rdx_6] = rcx_2;
                rax_1 = &rax_1[1];
            } while (r13_1 != rax_1);
            if (r10 <= r8)
            {
                break;
            }
        }
        if (r9 >= r8)
        {
            break;
        }
        int64_t rdx_7 = r9;
        int64_t* rax_4 = (arg1 + (rdx_7 << 3));
        int64_t r13_2 = (r11 + (((r14_2 - 1) + rdx_7) << 3));
        int64_t rcx_6 = (rbx - rdx_7);
        do
        {
            int64_t rcx_7 = *rax_4;
            *rax_4 = rax_4[rcx_6];
            rax_4[rcx_6] = rcx_7;
            rax_4 = &rax_4[1];
        } while (rax_4 != r13_2);
        r9 = (r9 + r14_2);
    }
    arg2[0xc] = rbp;
    uint64_t rax_3 = (rbp - r8);
    arg2[0xb] = (r12 + rax_3);
    return rax_3;
}

uint64_t sub_405e50(int32_t arg1, int64_t* arg2, char* arg3, int64_t* arg4, 
    int32_t* arg5, int32_t arg6, int32_t* arg7, int32_t arg8, int64_t arg9)
{
    char* r14 = *(arg7 + 0x20);
    char rax_1 = *r14;
    char* rbp = r14;
    uint64_t r13_1;
    if ((rax_1 == 0x3d || rax_1 == 0))
    {
        r13_1 = 0;
    }
    if ((rax_1 != 0x3d && rax_1 != 0))
    {
        while (true)
        {
            rbp = &rbp[1];
            char rax_2 = *rbp;
            if (rax_2 == 0)
            {
                break;
            }
            if (rax_2 == 0x3d)
            {
                break;
            }
            if ((!(rax_2 != 0 && rax_2 != 0x3d)))
            {
                /* nop */
            }
        }
        r13_1 = (rbp - r14);
    }
    int64_t* rbx = arg4;
    int64_t r12 = 0;
    char* rax_3 = *rbx;
    char* r15 = rax_3;
    uint64_t rax_14;
    if (rax_3 != 0)
    {
        int32_t rax_4;
        uint64_t rax_5;
        int32_t r8;
        while (true)
        {
            rax_4 = strncmp(r15, r14, r13_1);
            if (rax_4 == 0)
            {
                rax_5 = strlen(r15);
                if (rax_5 == r13_1)
                {
                    break;
                }
            }
            rbx = &rbx[4];
            r15 = *rbx;
            r8 = (r12 + 1);
            if (r15 == 0)
            {
                break;
            }
            r12 = r8;
        }
        if ((rax_4 == 0 && rax_5 == r13_1))
        {
            goto label_406080;
        }
        rbx = nullptr;
        int32_t var_44_1 = 0xffffffff;
        char* rdi_2 = rax_3;
        int64_t r15_1 = 0;
        int32_t var_50_1 = 0;
        int32_t var_40_1 = 0;
        char* var_70_1 = nullptr;
        int64_t* rbp_1 = arg4;
        do
        {
            if (strncmp(rdi_2, r14, r13_1) == 0)
            {
                if (rbx == 0)
                {
                    var_44_1 = r15_1;
                    rbx = rbp_1;
                }
                else if (((((arg6 != 0 || (arg6 == 0 && rbx[1] != rbp_1[1])) || ((arg6 == 0 && rbx[1] == rbp_1[1]) && rbx[2] != rbp_1[2])) || (((arg6 == 0 && rbx[1] == rbp_1[1]) && rbx[2] == rbp_1[2]) && rbx[3] != rbp_1[3])) && var_50_1 == 0))
                {
                    if (arg8 == 0)
                    {
                        var_50_1 = 1;
                        if (var_70_1 != 0)
                        {
                            goto label_405fa6;
                        }
                    }
                    else
                    {
                        if (var_70_1 != 0)
                        {
                            goto label_405fa6;
                        }
                        char* rax_36 = calloc(r8, 1);
                        var_70_1 = rax_36;
                        if (rax_36 == 0)
                        {
                            var_50_1 = 1;
                        }
                        else
                        {
                            var_40_1 = 1;
                            var_70_1[var_44_1] = 1;
                        label_405fa6:
                            var_70_1[r15_1] = 1;
                        }
                    }
                }
            }
            rbp_1 = &rbp_1[4];
            rdi_2 = *rbp_1;
            r15_1 = (r15_1 + 1);
        } while (rdi_2 != 0);
        if (((var_70_1 == 0 && var_50_1 == 0) && rbx != 0))
        {
            r12 = var_44_1;
        label_406080:
            int32_t rdx_3 = *arg7;
            *(arg7 + 0x20) = 0;
            int32_t rax_18 = (rdx_3 + 1);
            *arg7 = rax_18;
            int32_t rcx_4 = rbx[1];
            if (*rbp != 0)
            {
                if (rcx_4 != 0)
                {
                    *(arg7 + 0x10) = &rbp[1];
                }
                else
                {
                    if (arg8 != 0)
                    {
                        __fprintf_chk(stderr, 1, dcgettext(nullptr, "%s: option '%s%s' doesn't allow …", 5), *arg2, arg9, *rbx, arg4);
                    }
                    arg7[2] = rbx[3];
                    rax_14 = 0x3f;
                }
            }
            if ((rcx_4 == 1 && *rbp == 0))
            {
                if (rax_18 < arg1)
                {
                    *arg7 = (rdx_3 + 2);
                    *(arg7 + 0x10) = arg2[rax_18];
                }
                else
                {
                    if (arg8 != 0)
                    {
                        __fprintf_chk(stderr, 1, dcgettext(nullptr, "%s: option '%s%s' requires an ar…", 5), *arg2, arg9, *rbx, arg4);
                    }
                    arg7[2] = rbx[3];
                    char* rax_43;
                    rax_43 = *arg3 != 0x3a;
                    rax_14 = ((rax_43 * 5) + 0x3a);
                }
            }
            if ((((*rbp != 0 && rcx_4 != 0) || ((*rbp == 0 && rcx_4 == 1) && rax_18 < arg1)) || (*rbp == 0 && rcx_4 != 1)))
            {
                if (arg5 == 0)
                {
                    goto label_4060ae;
                }
                *arg5 = r12;
            label_4060ae:
                int32_t* rdx_4 = rbx[2];
                rax_14 = rbx[3];
                if (rdx_4 != 0)
                {
                    *rdx_4 = rax_14;
                    rax_14 = 0;
                }
            }
            goto label_4060cc;
        }
        if ((var_70_1 != 0 || (var_70_1 == 0 && var_50_1 != 0)))
        {
            if (arg8 != 0)
            {
                if (var_50_1 != 0)
                {
                    __fprintf_chk(stderr, 1, dcgettext(nullptr, "%s: option '%s%s' is ambiguous\n", 5), *arg2, arg9, r14, arg4);
                    r14 = *(arg7 + 0x20);
                }
                else
                {
                    flockfile(stderr);
                    __fprintf_chk(stderr, 1, dcgettext(nullptr, "%s: option '%s%s' is ambiguous; …", 5), *arg2, arg9, *(arg7 + 0x20), arg4);
                    uint64_t* const rdi_13 = stderr;
                    int64_t* rbx_4 = arg4;
                    char* rbp_5 = var_70_1;
                    void* r12_2 = &var_70_1[(r12 + 1)];
                    do
                    {
                        if (*rbp_5 != 0)
                        {
                            __fprintf_chk(rdi_13, 1, " '%s%s'", arg9, *rbx_4);
                            rdi_13 = stderr;
                        }
                        rbp_5 = &rbp_5[1];
                        rbx_4 = &rbx_4[4];
                    } while (r12_2 != rbp_5);
                    fputc(0xa, stderr);
                    funlockfile(stderr);
                    r14 = *(arg7 + 0x20);
                }
            }
            if (var_40_1 != 0)
            {
                free(var_70_1);
            }
            *(arg7 + 0x20) = &r14[strlen(r14)];
            *arg7 = (*arg7 + 1);
            arg7[2] = 0;
            rax_14 = 0x3f;
        label_4060cc:
            return rax_14;
        }
    }
    if ((arg6 != 0 && *(arg2[*arg7] + 1) != 0x2d))
    {
        rax_14 = 0xffffffff;
        if (strchr(arg3, rax_1) != 0)
        {
            goto label_4060cc;
        }
    }
    if (arg8 == 0)
    {
        goto label_40603e;
    }
    __fprintf_chk(stderr, 1, dcgettext(nullptr, "%s: unrecognized option '%s%s'\n", 5), *arg2, arg9, r14, arg4);
label_40603e:
    *(arg7 + 0x20) = 0;
    *arg7 = (*arg7 + 1);
    arg7[2] = 0;
    return 0x3f;
}

uint64_t sub_406470(int32_t arg1, int64_t* arg2, char* arg3, int64_t* arg4, 
    int32_t* arg5, int32_t arg6, int32_t* arg7, int32_t arg8)
{
    int32_t r13 = arg7[1];
    if (arg1 <= 0)
    {
        goto label_4066ac;
    }
    int32_t rax_1 = *arg7;
    int32_t rbp_1 = arg1;
    char* r15_1 = arg3;
    *(arg7 + 0x10) = 0;
    if (rax_1 == 0)
    {
        *arg7 = 1;
        rax_1 = 1;
    }
    uint32_t rax_3;
    char* rdx;
    if ((rax_1 == 0 || (rax_1 != 0 && arg7[6] == 0)))
    {
        arg7[0xc] = rax_1;
        arg7[0xb] = rax_1;
        *(arg7 + 0x20) = 0;
        char rax_2 = *r15_1;
        if (rax_2 == 0x2d)
        {
            arg7[0xa] = 2;
            r15_1 = &r15_1[1];
            rdx = nullptr;
        }
        else if (rax_2 == 0x2b)
        {
            arg7[0xa] = 0;
            r15_1 = &r15_1[1];
            rdx = nullptr;
        }
        else
        {
            rdx = nullptr;
            char* rax_16;
            if (arg8 == 0)
            {
                rax_16 = getenv("POSIXLY_CORRECT");
                if (rax_16 == 0)
                {
                    arg7[0xa] = 1;
                    rdx = *(arg7 + 0x20);
                }
                else
                {
                    rdx = *(arg7 + 0x20);
                }
            }
            if (((arg8 == 0 && rax_16 != 0) || arg8 != 0))
            {
                arg7[0xa] = 0;
            }
        }
        arg7[6] = 1;
        rax_3 = *r15_1;
    }
    if ((rax_1 != 0 && arg7[6] != 0))
    {
        rax_3 = *arg3;
        rdx = *(arg7 + 0x20);
    }
    if (((((rax_1 != 0 && arg7[6] != 0) && ((rax_3 - 0x2b) & 0xfd) != 0) || rax_1 == 0) || (rax_1 != 0 && arg7[6] == 0)))
    {
        if (rax_3 == 0x3a)
        {
            r13 = 0;
        }
        if (rdx == 0)
        {
            goto label_406538;
        }
    }
    if (((rax_1 != 0 && arg7[6] != 0) && ((rax_3 - 0x2b) & 0xfd) == 0))
    {
        char rax_4 = r15_1[1];
        r15_1 = &r15_1[1];
        if (rax_4 == 0x3a)
        {
            r13 = 0;
        }
        if (rdx == 0)
        {
        label_406538:
            int32_t rax_5 = *arg7;
            if (arg7[0xc] > rax_5)
            {
                arg7[0xc] = rax_5;
            }
            if (rax_5 < arg7[0xb])
            {
                arg7[0xb] = rax_5;
            }
            if (arg7[0xa] == 1)
            {
                int32_t rdx_5 = arg7[0xc];
                if (arg7[0xb] == rdx_5)
                {
                    if (rax_5 != rdx_5)
                    {
                        arg7[0xb] = rax_5;
                        rdx_5 = rax_5;
                    }
                }
                else if (rax_5 != rdx_5)
                {
                    sub_405d70(arg2, arg7);
                    rdx_5 = *arg7;
                }
                char* rcx_8;
                if (rbp_1 > rdx_5)
                {
                    int64_t rax_10 = rdx_5;
                    do
                    {
                        rcx_8 = arg2[rax_10];
                        int32_t rsi_5 = rax_10;
                        rdx_5 = rax_10;
                        if ((*rcx_8 == 0x2d && rcx_8[1] != 0))
                        {
                            break;
                        }
                        rdx_5 = (rsi_5 + 1);
                        rax_10 = (rax_10 + 1);
                        *arg7 = rdx_5;
                    } while (rbp_1 > rax_10);
                    if ((*rcx_8 == 0x2d && rcx_8[1] != 0))
                    {
                        rax_5 = *arg7;
                    }
                }
                if ((rbp_1 <= rdx_5 || (rbp_1 > rdx_5 && (!(*rcx_8 == 0x2d && rcx_8[1] != 0)))))
                {
                    rax_5 = rdx_5;
                }
                arg7[0xc] = rdx_5;
            }
            bool c_1 = rbp_1 < rax_5;
            bool z_1 = rbp_1 == rax_5;
            int64_t rcx_3;
            int32_t rdx_2;
            char* r14_1;
            if (z_1)
            {
                rbp_1 = arg7[0xc];
                rdx_2 = arg7[0xb];
            }
            else
            {
                rcx_3 = 3;
                void* const rdi = &data_4094e9;
                r14_1 = arg2[rax_5];
                char* rsi_1 = r14_1;
                while (rcx_3 != 0)
                {
                    char temp0_1 = *rsi_1;
                    char temp1_1 = *rdi;
                    c_1 = temp0_1 < temp1_1;
                    z_1 = temp0_1 == temp1_1;
                    rsi_1 = &rsi_1[1];
                    rdi = (rdi + 1);
                    rcx_3 = (rcx_3 - 1);
                    if ((!z_1))
                    {
                        break;
                    }
                }
                rcx_3 = ((!z_1) && (!c_1));
                rcx_3 = (rcx_3 - 0);
                if (rcx_3 == 0)
                {
                    rdx_2 = arg7[0xb];
                    int32_t rcx_4 = arg7[0xc];
                    int32_t rax_6 = (rax_5 + 1);
                    *arg7 = rax_6;
                    if (rdx_2 == rcx_4)
                    {
                        arg7[0xb] = rax_6;
                        rdx_2 = rax_6;
                    }
                    else if (rax_6 != rcx_4)
                    {
                        sub_405d70(arg2, arg7);
                        rdx_2 = arg7[0xb];
                    }
                    arg7[0xc] = rbp_1;
                    *arg7 = rbp_1;
                }
            }
            int32_t r14_2;
            if ((z_1 || ((!z_1) && rcx_3 == 0)))
            {
                if (rdx_2 == rbp_1)
                {
                    goto label_4066ac;
                }
                *arg7 = rdx_2;
            label_4066ac:
                r14_2 = -1;
            }
            if (((!z_1) && rcx_3 != 0))
            {
                char rcx_9;
                if (*r14_1 == 0x2d)
                {
                    rcx_9 = r14_1[1];
                    if (rcx_9 != 0)
                    {
                        if (arg4 == 0)
                        {
                            goto label_4067b0;
                        }
                        int32_t* var_88_2;
                        int32_t var_80_2;
                        void* const var_78_1;
                        if (rcx_9 == 0x2d)
                        {
                            *(arg7 + 0x20) = &r14_1[2];
                            var_78_1 = &data_4094e9;
                            var_80_2 = r13;
                            var_88_2 = arg7;
                            goto label_406820;
                        }
                        if (arg6 == 0)
                        {
                            goto label_4067b0;
                        }
                        if (r14_1[2] == 0)
                        {
                            if (strchr(r15_1, rcx_9) != 0)
                            {
                                goto label_4067b0;
                            }
                            goto label_406771;
                        }
                    label_406771:
                        *(arg7 + 0x20) = &r14_1[1];
                        int32_t rax_12 = sub_405e50(rbp_1, arg2, r15_1, arg4, arg5, arg6, arg7, r13, &data_4094ea);
                        r14_2 = rax_12;
                        if (rax_12 == 0xffffffff)
                        {
                            r14_1 = arg2[*arg7];
                        label_4067b0:
                            rdx = &r14_1[1];
                        label_4065f1:
                            *(arg7 + 0x20) = &rdx[1];
                            r14_2 = *rdx;
                            int32_t var_4c;
                            var_4c = r14_2;
                            char* rax_7 = strchr(r15_1, r14_2);
                            char rcx_5 = var_4c;
                            void* r8_1 = &rdx[1];
                            if (rdx[1] == 0)
                            {
                                *arg7 = (*arg7 + 1);
                            }
                            if (((rcx_5 - 0x3a) > 1 && rax_7 != 0))
                            {
                                char rcx_7 = rax_7[1];
                                if (((*rax_7 == 0x57 && arg4 != 0) && rcx_7 == 0x3b))
                                {
                                    if (rdx[1] != 0)
                                    {
                                        goto label_4067ee;
                                    }
                                    int64_t rax_14 = *arg7;
                                    if (rax_14 == rbp_1)
                                    {
                                        if (r13 != 0)
                                        {
                                            int64_t rbp_4 = *arg2;
                                            __fprintf_chk(stderr, 1, dcgettext(nullptr, "%s: option requires an argument …", 5), rbp_4, r14_2);
                                        }
                                        arg7[2] = r14_2;
                                        int64_t r14_4;
                                        r14_4 = *r15_1 != 0x3a;
                                        r14_2 = ((r14_4 * 5) + 0x3a);
                                    }
                                    else
                                    {
                                        r8_1 = arg2[rax_14];
                                    label_4067ee:
                                        *(arg7 + 0x20) = r8_1;
                                        arg6 = 0;
                                        *(arg7 + 0x10) = 0;
                                        var_78_1 = &data_409508;
                                        var_80_2 = r13;
                                        var_88_2 = arg7;
                                    label_406820:
                                        r14_2 = sub_405e50(rbp_1, arg2, r15_1, arg4, arg5, arg6, var_88_2, var_80_2, var_78_1);
                                    }
                                }
                                if ((((*rax_7 != 0x57 || (*rax_7 == 0x57 && arg4 == 0)) || ((*rax_7 == 0x57 && arg4 != 0) && rcx_7 != 0x3b)) && rcx_7 == 0x3a))
                                {
                                    char rdx_9 = rdx[1];
                                    if (rax_7[2] != 0x3a)
                                    {
                                        int64_t rax_17 = *arg7;
                                        if (rdx_9 != 0)
                                        {
                                            *(arg7 + 0x10) = r8_1;
                                            *arg7 = (rax_17 + 1);
                                        }
                                        else if (rbp_1 != rax_17)
                                        {
                                            int64_t rax_18 = arg2[rax_17];
                                            *arg7 = (rax_17 + 1);
                                            *(arg7 + 0x10) = rax_18;
                                        }
                                        else
                                        {
                                            if (r13 != 0)
                                            {
                                                int64_t rbp_3 = *arg2;
                                                __fprintf_chk(stderr, 1, dcgettext(nullptr, "%s: option requires an argument …", 5), rbp_3, r14_2);
                                            }
                                            arg7[2] = r14_2;
                                            int64_t r14_3;
                                            r14_3 = *r15_1 != 0x3a;
                                            r14_2 = ((r14_3 * 5) + 0x3a);
                                        }
                                    }
                                    else if (rdx_9 == 0)
                                    {
                                        *(arg7 + 0x10) = 0;
                                    }
                                    else
                                    {
                                        *(arg7 + 0x10) = r8_1;
                                        *arg7 = (*arg7 + 1);
                                    }
                                    *(arg7 + 0x20) = 0;
                                }
                            }
                            if (((rcx_5 - 0x3a) <= 1 || ((rcx_5 - 0x3a) > 1 && rax_7 == 0)))
                            {
                                if (r13 == 0)
                                {
                                    goto label_4068dd;
                                }
                                int64_t rbp_2 = *arg2;
                                __fprintf_chk(stderr, 1, dcgettext(nullptr, "%s: invalid option -- '%c'\n", 5), rbp_2, r14_2);
                            label_4068dd:
                                arg7[2] = r14_2;
                                r14_2 = 0x3f;
                            }
                        }
                    }
                }
                if ((*r14_1 != 0x2d || (*r14_1 == 0x2d && rcx_9 == 0)))
                {
                    if (arg7[0xa] == 0)
                    {
                        goto label_4066ac;
                    }
                    *(arg7 + 0x10) = r14_1;
                    r14_2 = 1;
                    *arg7 = (rax_5 + 1);
                }
            }
            if ((((z_1 || ((!z_1) && rcx_3 == 0)) && rdx_2 != rbp_1) || ((!z_1) && rcx_3 != 0)))
            {
                return r14_2;
            }
        }
    }
    if (((((((rax_1 != 0 && arg7[6] != 0) && ((rax_3 - 0x2b) & 0xfd) != 0) || rax_1 == 0) || (rax_1 != 0 && arg7[6] == 0)) && rdx != 0) || ((rax_1 != 0 && arg7[6] != 0) && ((rax_3 - 0x2b) & 0xfd) == 0)))
    {
        if (*rdx == 0)
        {
            goto label_406538;
        }
        goto label_4065f1;
    }
}

uint64_t sub_406aa0(int32_t arg1, int64_t* arg2, char* arg3)
{
    return _getopt_internal(arg1, arg2, arg3, nullptr, nullptr, 0, 1);
}

uint64_t sub_406b40(uint32_t* arg1, char* arg2, uint64_t arg3, mbstate_t* arg4)
{
    uint64_t rbx_1;
    char* rbp_1;
    uint32_t* r12_1;
    if (arg2 == 0)
    {
        arg3 = 1;
        rbp_1 = &data_4094ec[0x1b];
    }
    else
    {
        rbx_1 = -2;
        if (arg3 != 0)
        {
            r12_1 = arg1;
            rbp_1 = arg2;
            if (arg1 != 0)
            {
                goto label_406b6a;
            }
        }
    }
    if ((arg2 == 0 || (arg2 != 0 && arg3 != 0)))
    {
        void var_1c;
        r12_1 = &var_1c;
    label_406b6a:
        uint64_t rax_1 = mbrtowc(r12_1, rbp_1, arg3, arg4);
        rbx_1 = rax_1;
        if ((rax_1 > -3 && sub_406e80() == 0))
        {
            rbx_1 = 1;
            *r12_1 = *rbp_1;
        }
    }
    return rbx_1;
}

char* sub_406bc0(char* arg1, int32_t arg2, int64_t arg3)
{
    char r8 = arg2;
    if (arg3 != 0)
    {
        if ((arg1 & 7) != 0)
        {
            if (arg2 == *arg1)
            {
            label_406cc0:
                return arg1;
            }
            do
            {
                arg1 = &arg1[1];
                int64_t temp0_1 = arg3;
                arg3 = (arg3 - 1);
                if (temp0_1 == 1)
                {
                    return 0;
                }
                if ((arg1 & 7) == 0)
                {
                    break;
                }
            } while (*arg1 != r8);
            if ((arg1 & 7) != 0)
            {
                goto label_406cc0;
            }
        }
        int64_t r9_5 = ((arg2 << 8) | arg2);
        int64_t r9_6 = (r9_5 | (r9_5 << 0x10));
        int64_t r9_7 = (r9_6 | (r9_6 << 0x20));
        int64_t rax_8;
        if (arg3 > 7)
        {
            rax_8 = (*arg1 ^ r9_7);
            if ((((!rax_8) & (rax_8 - 0x101010101010101)) & 0x8080808080808080) == 0)
            {
                int64_t rax_12;
                do
                {
                    arg3 = (arg3 - 8);
                    arg1 = &arg1[8];
                    if (arg3 <= 7)
                    {
                        break;
                    }
                    rax_12 = (*arg1 ^ r9_7);
                } while ((((!rax_12) & (rax_12 - 0x101010101010101)) & 0x8080808080808080) == 0);
            }
        }
        if ((((arg3 <= 7 || (arg3 > 7 && (((!rax_8) & (rax_8 - 0x101010101010101)) & 0x8080808080808080) != 0)) || (((arg3 > 7 && (((!rax_8) & (rax_8 - 0x101010101010101)) & 0x8080808080808080) == 0) && arg3 <= 7) && arg3 != 0)) || ((arg3 > 7 && (((!rax_8) & (rax_8 - 0x101010101010101)) & 0x8080808080808080) == 0) && arg3 > 7)))
        {
            if (arg2 == *arg1)
            {
                goto label_406cc0;
            }
            char* rcx_3 = &arg1[1];
            void* rdi = &arg1[arg3];
            while (true)
            {
                char* rax_15 = rcx_3;
                if (rcx_3 == rdi)
                {
                    return 0;
                }
                rcx_3 = &rcx_3[1];
                if (rcx_3[-1] == r8)
                {
                    return rax_15;
                }
            }
        }
    }
    return 0;
}

uint64_t sub_406cd0(FILE* arg1)
{
    int32_t* rbp = arg1;
    uint64_t rax = __fpending(arg1);
    int32_t rbx_1 = (*rbp & 0x20);
    uint64_t rax_1 = sub_405c20(rbp);
    if ((rbx_1 != 0 && rax_1 == 0))
    {
        *__errno_location() = 0;
        rax_1 = 0xffffffff;
    }
    if (((rbx_1 != 0 && rax_1 != 0) || ((rbx_1 == 0 && rax_1 != 0) && rax != 0)))
    {
        rax_1 = 0xffffffff;
    }
    if (((rbx_1 == 0 && rax_1 != 0) && rax == 0))
    {
        int32_t* rax_2;
        rax_2 = *__errno_location() != 9;
        rax_1 = (-rax_2);
    }
    return rax_1;
}

uint64_t sub_406d30(int32_t* arg1, off_t arg2)
{
    if (arg2 == 0)
    {
        return 0;
    }
    off_t rbx = arg2;
    int64_t rax = sub_407450(arg1);
    int64_t rbp = rax;
    off_t var_1028;
    if (rax != 0)
    {
        while (true)
        {
            int64_t rax_1 = sub_405ce0(arg1, &var_1028);
            off_t rax_2;
            off_t temp3_1;
            if (rax_1 != 0)
            {
                rax_2 = var_1028;
                if (rax_2 != 0)
                {
                    if (rax_2 > rbx)
                    {
                        rax_2 = rbx;
                    }
                    *(arg1 + 8) = (*(arg1 + 8) + rax_2);
                    temp3_1 = rbx;
                    rbx = (rbx - rax_2);
                    if (temp3_1 != rax_2)
                    {
                        int64_t temp4_1 = rbp;
                        rbp = (rbp - rax_2);
                        if (temp4_1 == rax_2)
                        {
                            break;
                        }
                    }
                }
            }
            if (((rax_1 == 0 || (rax_1 != 0 && rax_2 == 0)) || ((rax_1 != 0 && rax_2 != 0) && temp3_1 != rax_2)))
            {
                if (fgetc(arg1) == 0xffffffff)
                {
                    goto label_406dc6;
                }
                off_t temp0_1 = rbx;
                rbx = (rbx - 1);
                if (temp0_1 != 1)
                {
                    int64_t temp2_1 = rbp;
                    rbp = (rbp - 1);
                    if (temp2_1 == 1)
                    {
                        break;
                    }
                    continue;
                }
            }
            return 0;
        }
    }
    int32_t rax_8 = fileno(arg1);
    uint64_t rax_6;
    off64_t rax_10;
    if (rax_8 >= 0)
    {
        rax_10 = lseek(rax_8, 0, 1);
        if (rax_10 >= 0)
        {
            rax_6 = sub_405d10(arg1, rbx, 1);
        }
    }
    if ((rax_8 < 0 || (rax_8 >= 0 && rax_10 < 0)))
    {
        while (true)
        {
            uint64_t rbp_1 = 0x1000;
            FILE* r8_1 = arg1;
            off_t* rdi_4 = &var_1028;
            if (rbx <= 0x1000)
            {
                rbp_1 = rbx;
            }
            if (rbp_1 > __fread_chk(rdi_4, 0x1000, 1, rbp_1, r8_1))
            {
                break;
            }
            off_t temp1_1 = rbx;
            rbx = (rbx - rbp_1);
            if (temp1_1 == rbp_1)
            {
                return 0;
            }
        }
    label_406dc6:
        int32_t rax_4;
        rax_4 = ferror(arg1) != 0;
        rax_6 = (-rax_4);
    }
    if ((((rax_8 >= 0 && rax_10 >= 0) || rax_8 < 0) || (rax_8 >= 0 && rax_10 < 0)))
    {
        return rax_6;
    }
}

int64_t sub_406e80()
{
    void var_8;
    int64_t rcx;
    int64_t rdx;
    int32_t rdi;
    uint64_t r8;
    ssize_t r9;
    char* rax = setlocale(rdi, nullptr, rdx, rcx, r8, r9, var_8, __return_addr);
    int64_t rax_1 = 1;
    bool c = false;
    bool z = (rax & rax) == 0;
    if ((!z))
    {
        int64_t rcx_1 = 2;
        void* const rdi_1 = &data_40950c;
        char* rsi_1 = rax;
        while (rcx_1 != 0)
        {
            char temp0_1 = *rsi_1;
            char temp1_1 = *rdi_1;
            c = temp0_1 < temp1_1;
            z = temp0_1 == temp1_1;
            rsi_1 = &rsi_1[1];
            rdi_1 = (rdi_1 + 1);
            rcx_1 = (rcx_1 - 1);
            if ((!z))
            {
                break;
            }
        }
        rcx_1 = ((!z) && (!c));
        rcx_1 = (rcx_1 - 0);
        rax_1 = 0;
        bool c_1 = false;
        bool z_1 = (rcx_1 & rcx_1) == 0;
        if ((!z_1))
        {
            void* const rdi_2 = "POSIX";
            int64_t rcx_2 = 6;
            char* rsi_2 = rax;
            while (rcx_2 != 0)
            {
                char temp2_1 = *rsi_2;
                char temp3_1 = *rdi_2;
                c_1 = temp2_1 < temp3_1;
                z_1 = temp2_1 == temp3_1;
                rsi_2 = &rsi_2[1];
                rdi_2 = (rdi_2 + 1);
                rcx_2 = (rcx_2 - 1);
                if ((!z_1))
                {
                    break;
                }
            }
            rax_1 = ((!z_1) && (!c_1));
            rax_1 = (rax_1 - 0);
            rax_1 = rax_1 != 0;
            return rax_1;
        }
    }
    return rax_1;
}

char* sub_406ee0()
{
    char* rax = nl_langinfo(0xe);
    char* r15 = data_60b4d8;
    char* rbx = rax;
    if (rax == 0)
    {
        rbx = &data_4094ec[0x1b];
    }
    if (r15 == 0)
    {
        char* rax_6 = getenv("CHARSETALIASDIR");
        char* r13_1 = rax_6;
        uint64_t rdi_7;
        uint64_t r12_1;
        uint64_t r14_1;
        if ((rax_6 == 0 || (rax_6 != 0 && *rax_6 == 0)))
        {
            rdi_7 = 0x73;
            r12_1 = 0x65;
            r14_1 = 0x64;
            r13_1 = "/home/dongkwan/data/scripts/gnu_…";
        }
        char* rbp_3;
        if ((rax_6 != 0 && *rax_6 != 0))
        {
            uint64_t rax_7 = strlen(rax_6);
            r12_1 = rax_7;
            r14_1 = rax_7;
            uint64_t rdi_6;
            if (rax_7 == 0)
            {
                rdi_6 = 0xe;
            }
            else if (r13_1[(rax_7 - 1)] != 0x2f)
            {
                rdi_7 = (rax_7 + 0xf);
                r12_1 = (r12_1 + 1);
            }
            else
            {
                rdi_6 = (rax_7 + 0xe);
            }
            if ((rax_7 == 0 || (rax_7 != 0 && r13_1[(rax_7 - 1)] == 0x2f)))
            {
                char* rax_8 = malloc(rdi_6);
                rbp_3 = rax_8;
                if (rax_8 == 0)
                {
                    goto label_406fdc;
                }
                memcpy(rbp_3, r13_1, r14_1);
                goto label_407026;
            }
        }
        char* rax_9 = malloc(rdi_7);
        rbp_3 = rax_9;
        if (rax_9 == 0)
        {
        label_406fdc:
            r15 = &data_4094ec[0x1b];
        }
        else
        {
            memcpy(rbp_3, r13_1, r14_1);
            rbp_3[r14_1] = 0x2f;
        label_407026:
            int64_t* r12_2 = &rbp_3[r12_1];
            *r12_2 = 0x2e74657372616863;
            *(r12_2 + 0xc) = 0x73;
            r12_2[1] = 0x61696c61;
            int32_t rax_11 = open(rbp_3, 0);
            void* var_d0_1;
            FILE* rax_12;
            if (rax_11 >= 0)
            {
                r15 = nullptr;
                rax_12 = fdopen(rax_11, &data_40764e);
                var_d0_1 = nullptr;
                if (rax_12 == 0)
                {
                    close(rax_11);
                }
                else
                {
                    while (true)
                    {
                        char* rax_14 = rax_12->_IO_read_ptr;
                        uint32_t rdi_13;
                        if (rax_14 < rax_12->_IO_read_end)
                        {
                            do
                            {
                                rax_12->_IO_read_ptr = &rax_14[1];
                                rdi_13 = *rax_14;
                            label_4070dc:
                                if ((rdi_13 - 9) <= 1)
                                {
                                    break;
                                }
                                if (rdi_13 == 0x20)
                                {
                                    break;
                                }
                                if (rdi_13 == 0x23)
                                {
                                    break;
                                }
                                ungetc(rdi_13, rax_12);
                                int64_t var_b8;
                                int64_t var_78;
                                if (fscanf(rax_12, "%50s %50s", &var_b8, &var_78) <= 1)
                                {
                                    goto label_40726b;
                                }
                                int64_t* rcx_2 = &var_b8;
                            label_407118:
                                int32_t rdx_5 = *rcx_2;
                                rcx_2 = (rcx_2 + 4);
                                uint32_t rax_20 = (((rdx_5 - 0x1010101) & (!rdx_5)) & 0x80808080);
                                if (rax_20 == 0)
                                {
                                    goto label_407118;
                                }
                                uint32_t rdx_8 = (rax_20 >> 0x10);
                                int32_t temp0_1 = (rax_20 & 0x8080);
                                bool cond:2_1 = temp0_1 == 0;
                                if (temp0_1 == 0)
                                {
                                    rax_20 = rdx_8;
                                }
                                void* rdx_9 = (rcx_2 + 2);
                                if (cond:2_1)
                                {
                                    rcx_2 = rdx_9;
                                }
                                char rdx_10 = rax_20;
                                int64_t* rdx_11 = &var_78;
                                void* rcx_4 = ((rcx_2 - 3) - &var_b8);
                            label_407152:
                                int32_t rsi_5 = *rdx_11;
                                rdx_11 = (rdx_11 + 4);
                                uint32_t rax_23 = (((rsi_5 - 0x1010101) & (!rsi_5)) & 0x80808080);
                                if (rax_23 == 0)
                                {
                                    goto label_407152;
                                }
                                uint32_t rsi_8 = (rax_23 >> 0x10);
                                int32_t temp2_1 = (rax_23 & 0x8080);
                                bool cond:3_1 = temp2_1 == 0;
                                if (temp2_1 == 0)
                                {
                                    rax_23 = rsi_8;
                                }
                                void* rsi_9 = (rdx_11 + 2);
                                if (cond:3_1)
                                {
                                    rdx_11 = rsi_9;
                                }
                                char rsi_10 = rax_23;
                                void* rdx_13 = ((rdx_11 - 3) - &var_78);
                                void* rax_24 = (rcx_4 + rdx_13);
                                void* var_c0 = rdx_13;
                                char* rax_25;
                                void* rcx_6;
                                void* rdx_14;
                                if (var_d0_1 != 0)
                                {
                                    void* rax_31 = (rax_24 + var_d0_1);
                                    var_d0_1 = (rax_31 + 2);
                                    rax_25 = realloc(r15, (rax_31 + 3));
                                    rdx_14 = var_c0;
                                    rcx_6 = rcx_4;
                                }
                                else
                                {
                                    var_d0_1 = (rax_24 + 2);
                                    rax_25 = malloc((rax_24 + 3));
                                    rcx_6 = rcx_4;
                                    rdx_14 = var_c0;
                                }
                                if (rax_25 == 0)
                                {
                                    char* rdi_26 = r15;
                                    r15 = &data_4094ec[0x1b];
                                    free(rdi_26);
                                    sub_405c20(rax_12);
                                    goto label_407067;
                                }
                                int32_t rcx_7 = (rcx_6 + 1);
                                void* rdi_17 = (var_d0_1 - rdx_14);
                                void* rsi_13 = (((-2 - rcx_6) + rdi_17) + rax_25);
                                if (rcx_7 >= 8)
                                {
                                    *rsi_13 = var_b8;
                                    uint64_t r9_3 = rcx_7;
                                    *((rsi_13 + r9_3) - 8) = *(&var_c0 + r9_3);
                                    void* r9_5 = ((rsi_13 + 8) & 0xfffffffffffffff8);
                                    void* rsi_16 = (rsi_13 - r9_5);
                                    int64_t* r10_2 = (&var_b8 - rsi_16);
                                    int32_t rcx_11 = ((rcx_7 + rsi_16) & 0xfffffff8);
                                    if (rcx_11 >= 8)
                                    {
                                        int32_t rcx_12 = (rcx_11 & 0xfffffff8);
                                        int32_t rsi_17 = 0;
                                    label_40732b:
                                        uint64_t r8_1 = rsi_17;
                                        rsi_17 = (rsi_17 + 8);
                                        *(r9_5 + r8_1) = *(r10_2 + r8_1);
                                        if (rsi_17 < rcx_12)
                                        {
                                            goto label_40732b;
                                        }
                                    }
                                }
                                else if ((rcx_7 & 4) != 0)
                                {
                                    uint64_t rcx_15 = rcx_7;
                                    *rsi_13 = var_b8;
                                    *((rsi_13 + rcx_15) - 4) = *(&*var_c0[4] + rcx_15);
                                }
                                else if (rcx_7 != 0)
                                {
                                    *rsi_13 = var_b8;
                                    if ((rcx_7 & 2) != 0)
                                    {
                                        uint64_t rcx_16 = rcx_7;
                                        *((rsi_13 + rcx_16) - 2) = *(&*var_c0[6] + rcx_16);
                                    }
                                }
                                int32_t rdx_15 = (rdx_14 + 1);
                                void* rcx_8 = ((rax_25 + rdi_17) - 1);
                                if (rdx_15 >= 8)
                                {
                                    *rcx_8 = var_78;
                                    uint64_t rsi_19 = rdx_15;
                                    void var_80;
                                    *((rcx_8 + rsi_19) - 8) = *(&var_80 + rsi_19);
                                    void* rdi_24 = ((rcx_8 + 8) & 0xfffffffffffffff8);
                                    void* rcx_13 = (rcx_8 - rdi_24);
                                    int64_t* r9_6 = (&var_78 - rcx_13);
                                    int32_t rdx_19 = ((rdx_15 + rcx_13) & 0xfffffff8);
                                    if (rdx_19 >= 8)
                                    {
                                        int32_t rdx_20 = (rdx_19 & 0xfffffff8);
                                        int32_t rcx_14 = 0;
                                    label_407379:
                                        uint64_t rsi_20 = rcx_14;
                                        rcx_14 = (rcx_14 + 8);
                                        *(rdi_24 + rsi_20) = *(r9_6 + rsi_20);
                                        if (rcx_14 < rdx_20)
                                        {
                                            goto label_407379;
                                        }
                                    }
                                }
                                else if ((rdx_15 & 4) != 0)
                                {
                                    uint64_t rdx_21 = rdx_15;
                                    *rcx_8 = var_78;
                                    void var_7c;
                                    *((rcx_8 + rdx_21) - 4) = *(&var_7c + rdx_21);
                                }
                                else if (rdx_15 != 0)
                                {
                                    *rcx_8 = var_78;
                                    if ((rdx_15 & 2) != 0)
                                    {
                                        uint64_t rsi_23 = rdx_15;
                                        void var_7a;
                                        *((rcx_8 + rsi_23) - 2) = *(&var_7a + rsi_23);
                                    }
                                }
                                r15 = rax_25;
                                rax_14 = rax_12->_IO_read_ptr;
                            } while (rax_14 < rax_12->_IO_read_end);
                            if (((!((rdi_13 - 9) > 1 && rdi_13 == 0x23)) && (rdi_13 - 9) <= 1))
                            {
                                continue;
                            }
                            if (((rdi_13 - 9) > 1 && rdi_13 == 0x23))
                            {
                                int32_t rax_29;
                                bool rdx_17;
                                do
                                {
                                    char* rax_30 = rax_12->_IO_read_ptr;
                                    if (rax_30 < rax_12->_IO_read_end)
                                    {
                                        rax_12->_IO_read_ptr = &rax_30[1];
                                        rax_29 = *rax_30;
                                        rdx_17 = true;
                                    }
                                    else
                                    {
                                        rax_29 = __uflow(rax_12);
                                        rdx_17 = rax_29 != 0xffffffff;
                                    }
                                    if (rax_29 == 0xa)
                                    {
                                        break;
                                    }
                                } while (rdx_17 != 0);
                                if (rax_29 == 0xffffffff)
                                {
                                    break;
                                }
                                continue;
                            }
                        }
                        uint32_t rax_26 = __uflow(rax_12);
                        rdi_13 = rax_26;
                        if (rax_26 != 0xffffffff)
                        {
                            goto label_4070dc;
                        }
                        break;
                    }
                label_40726b:
                    sub_405c20(rax_12);
                    if (var_d0_1 != 0)
                    {
                        *(r15 + var_d0_1) = 0;
                    }
                }
            }
            if ((((rax_11 >= 0 && rax_12 == 0) || rax_11 < 0) || ((rax_11 >= 0 && rax_12 != 0) && var_d0_1 == 0)))
            {
                r15 = &data_4094ec[0x1b];
            }
        label_407067:
            free(rbp_3);
        }
        data_60b4d8 = r15;
    }
    char rbp_2 = *r15;
    if (rbp_2 != 0)
    {
        while (true)
        {
            if ((strcmp(rbx, r15) != 0 && (rbp_2 != 0x2a || (rbp_2 == 0x2a && r15[1] != 0))))
            {
                void* rbp_1 = &r15[(strlen(r15) + 1)];
                r15 = ((rbp_1 + strlen(rbp_1)) + 1);
                rbp_2 = *r15;
                if (rbp_2 == 0)
                {
                    break;
                }
                continue;
            }
            rbx = &r15[(strlen(r15) + 1)];
            break;
        }
    }
    if (*rbx == 0)
    {
        rbx = "ASCII";
    }
    return rbx;
}

int64_t sub_407450(int32_t* arg1)
{
    int64_t rax_1 = 0;
    if (*(arg1 + 0x28) <= *(arg1 + 0x20))
    {
        rax_1 = (*(arg1 + 0x10) - *(arg1 + 8));
        if ((*arg1 & 0x100) != 0)
        {
            rax_1 = (rax_1 + (*(arg1 + 0x58) - *(arg1 + 0x48)));
        }
    }
    return rax_1;
}

void init()
{
    _init();
}

void fini()
{
    return;
}

int64_t sub_4074f0(void (* arg1)(void*))
{
    int64_t rdx = data_60b228;
    /* tailcall */
    return __cxa_atexit(arg1, nullptr, rdx);
}

int64_t sub_407508(int64_t arg1, int64_t arg2, int64_t arg3)
{
    int64_t* rbx = &data_60ae30;
    while (true)
    {
        int64_t rax_1 = *rbx;
        if (rax_1 == -1)
        {
            break;
        }
        rax_1();
        rbx = (rbx - 8);
    }
    return arg3;
}

